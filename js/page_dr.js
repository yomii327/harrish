...
var TabArr = new Array('General', 'Architectural', 'Structure', 'Services', 'Concrete &amp; PT', 'Civil / Landscaping', 'ESD / Green Star', 'Survey', 'Markups', 'Shop Drawings');

if($('#project_Id').val()=='242' || $('#project_Id').val()=='243'){
	var TabArr = new Array('Architectural', 'Structural', 'Mechanical', 'Civil', 'Electrical', 'Hydraulics', 'Fire Services', 'Landscaping', 'Spec., Sched. & Reports', 'Models', 'Shop Drawings');
}
	...
	switch(arrayElement){
		...
		case 'Architectural' :
			if($('#project_Id').val()=='242' || $('#project_Id').val()=='243'){
				...
				var TabArrTwo = new Array('shop drawings');
			}else{
				var TabArrTwo = new Array('Lighting', 'Tenancy Fitout', 'Penthouse Architecture', 'Drawings', 'Specifications', 'Reports &amp; Schedules', 'Plans', 'Elevations', 'Sections', 'RCP\'s', 'Details', 'Document Transmittal');
			}
		break;
		...
		case 'Structure' :
			if($('#project_Id').val()=='242' || $('#project_Id').val()=='243'){
				...
				var TabArrTwo = new Array('Drawings', 'Specifications', 'Reports &amp; Schedules', 'Civil', 'Site Inspection', 'Project Advice Notice', 'Document Transmittal');
			}
		break;

		case 'Services' :
			var TabArrTwo = new Array('Acoustic', 'Drawings', 'Specifications', 'Reports &amp; Schedules', 'Mechanical', 'Electrical', 'Hydraulic', 'Fire', 'Lifts', 'Document Transmittal');
		break;
		
		case 'Concrete &amp; PT' :
			var TabArrTwo = new Array('Concrete Profile', 'Bottom Reinforcement', 'Top Reinforcement', 'Sheer Reinforcement', 'PT Design', 'Details', 'Site Inspection', 'Document Transmittal');
		break;
		...
		case 'Civil / Landscaping' :
			var TabArrTwo = new Array('Civil', 'Landscaping', 'Drawings', 'Specification', 'Reports &amp; Schedules', 'Plans', 'Elevations', 'Sections', 'RCP\'s', 'Details', 'Photos', 'Document Transmittal');
		break;
		
		case 'ESD / Green Star' :
			var TabArrTwo = new Array('Document Transmittal');
		break;
		
		case 'Survey' :
			var TabArrTwo = new Array('SOS', 'Document Transmittal');
		break;
				
		case 'Lighting' :
			var TabArrTwo = new Array('Drawings', 'Specification', 'Document Transmittal');
		break;

		case 'Tenancy Fitout' :
			var TabArrTwo = new Array('Drawings', 'Specification', 'Reports &amp; Schedules', 'Plans', 'Elevations', 'Sections', 'RCP\'s', 'Details', 'Photos', 'Document Transmittal');
		break;
		...
		/*case 'Shop Drawings' :
			var TabArrTwo = new Array('Precast', 'Structure', 'Structural Steel', 'Mechanical', 'Electrical', 'Fire', 'Hydraulics', 'Lifts', 'Facade', 'Joinery', 'Roof Access', 'Miscellaneous', 'Document Transmittal');
		break;*/
		
		case 'Spec., Sched. & Reports':
			var TabArrTwo = new Array('Acoustic Documents', 'Architectural', 'Building Surveyor', 'Electrical', 'Fire', 'Geotechnical', 'Traffic Report', 'WMP Plan');
		break;
...
*/
		case 'Shop Drawings' :
			var TabArrTwo = new Array('Acoustic Documents', 'Architectural', 'Building Surveyor', 'Electrical', 'Fire', 'Geotechnical', 'Traffic Report', 'WMP Plan', 'Lift');
		break;
...
var attrTabs = '<ul id="attributeTabs">';
	for (i=0; i<TabArr.length; i++){
		if(TabArr[i]=='Markups'){
			...
			var idCreate = TabArr[i].replace(/ /g,'');
			if(idCreate == "Concrete&amp;PT"){
				idCreate = "ConcretePT";
			}
			if(idCreate == "Civil/Landscaping"){
				idCreate = "CivilLandscaping";
			}
			if(idCreate == "ESD/GreenStar"){
				idCreate = "ESDGreenStar";
			}
			attrTabs += '<li class="Admin"><a href="javascript:searchDrawPdf(\''+TabArr[i]+'\');" class="allElements" id="'+idCreate+'_id"><span>';
		}
...
//			TabArrTwo[j] = TabArrTwo[j].replace(/'/g, "\\'");
			attrTabs += '<li class=""><span onclick="searchDrawPdfSecAttr(\''+TabArr[i]+'\', \''+TabArrTwo[j].replace(/'/g, "\\'")+'\');">'+TabArrTwo[j]+'</span></li>';
		}
		attrTabs += '</ul>';
	}
	attrTabs += '</ul>';

var responseText = '<form id="allTaskTable" name="allTaskTable"><table cellpadding="0" cellspacing="0" border="0" class="display" id="drawingDefault" width="100%"><thead><tr><th></th><th><input id="checkall" type="checkbox" onclick="toggleCheck(this);" /></th><th width="70px">Document Number</th><th width="100px">Document Description</th><th>Rev.</th><th>Attribute 1</th><th>Attribute 2</th><th>Attribute 3</th><th>Tag</th><th width="50px">Status</th><th width="80px">Download on iPad</th>';
responseText += '<th>Approved</th><th>Uploaded Date</th><th>Action</th></tr></thead><tbody><tr><td colspan="8" class="dataTables_empty">Loading data from server</td></tr></tbody></table></from>';



function showRevisions(pdfID,M){
	modalPopup(align, top1, 690, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'drawing_registration_show_v1.php?pdfID='+pdfID+'&prev='+M, loadingImage);
}
function addNewRegister(){
	modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'add_drawing_register_v1.php?&name='+Math.random(), loadingImage, afterLoadRegistration);
}
function addNewRegisterRevision(regID){
	modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'add_drawing_register_revision_v1.php?tableID='+regID+'&name='+Math.random(), loadingImage, afterLoad);
}
...
function updateDrawingRegisterData(){
	...
	showProgress();
	$.post('edit_drawing_register_v1.php?antiqueID='+Math.random(), $('#editDrawingForm').serialize()).done(function(data) {
		hideProgress();
...
function afterLoadRegistration(){
	...
	var config = {
		...
		uploadUrl: "add_drawing_register_v1.php?antiqueID="+Math.random(),// Server side upload url
		ajaxCheckUploadUrl: "add_drawing_register_v1.php?i=1&superAntiqueID=" + Math.random()
	}
	deadlockPDF = true;
	var revision4check = "";	
...
function afterLoad(){
	...
	var config = {
		...
		form: "addDrawingForm",// Form ID
		dragArea: "innerDivDrager",// Upload Area ID
		uploadUrl: "add_drawing_register_revision_v1.php?antiqueID="+Math.random()// Server side upload url
	}
	initMultiUploader(config);
	deadlockPDF = true;
	var revision4check = "";	
...
function RefreshTable(){
	//$.getJSON("drawing_register_data_table_v1.php?"+params, null, function( json ){
	$.getJSON("drawing_register_data_table_v1.php?req=pmb", null, function( json ){
...
function resetSearch(){
	...
	for (i=0; i<TabArr.length; i++){
		...
		attrTabs += '<li class="Admin"><a href="javascript:searchDrawPdf(\''+TabArr[i]+'\');" class="allElements" id="'+idCreate+'_id"><span>'+TabArr[i]+'</span></a>';
	...
	var oTable = $('#drawingDefault').dataTable( {
		...
		"bServerSide": true,
		"bRetrieve": true,
		"sAjaxSource": "drawing_register_data_table_v1.php?attr1="+encodeURIComponent(attribute1Arr)+"&name="+Math.random(),
		"bStateSave": true,
...
function bulkUploadRegisters(){
	modalPopup(align, top1, 1000, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'add_drawing_register_bulk_v1.php?&name='+Math.random(), loadingImage, bulkRegistration);
}
...
function bulkRegistration(){
	var config = {
		...
		uploadUrl: "add_drawing_register_bulk_v1.php",// +new Date().getTime()Server side upload url
		ajaxCheckUploadUrl: "add_drawing_register_bulk_v1.php?i=1&superAntiqueID=" + Math.random()
	}
	$('select#drawingattribute1').change(function(){
		var currValue = $(this).val();
		var outputStr = '<option value="">Select</option>';
...
//Filter Array
	...
	jPrompt('Zip File Name:', '', 'Download', function(r) {
		...
		$.post("download_document_register_files_v1.php", {projectID:projectID, customName:curstomName, fileIds:taskArray, uniqueId:Math.random()}).done(function(data) {
			hideProgress();
			var jsonResult = JSON.parse(data);	
			if(jsonResult.status){
...
function runRerporDrawingRegister(projectID){
	...
	var attribute1 = $('#drawattribute1').val().trim();
	showProgress();
	$.get("drawing_registration_report_v1.php", {projID:projectID, revisionType:revisionType, sortBy:sortBy, attribute1:attribute1, spectialCon:'Y', name:Math.random()}).done(function(data) {
		hideProgress();
...
function showHistory(pdfID, projID){
	modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'drawing_history_table_v1.php?&name='+Math.random(), loadingImage, function() {loadData(pdfID, projID);});
}
function loadData(pdfID, projID){
	$('#example_server').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "drawing_history_ajax_table_v1.php?pdfID="+pdfID+"&projectID="+projID,
		"bStateSave": true,
		"bFilter": false,
...
function downloadPDF(projectID){
	...
	var attribute1 = $('#drawattribute1').val().trim();
	showProgress();
	$.get("pdf/drawing_registration_report_manager_v1.php", {projID:projectID, revisionType:revisionType, sortBy:sortBy, attribute1:attribute1, spectialCon:'Y', name:Math.random()}).done(function(data) {
		hideProgress();
		$('#mainContainer').html(data);
	});	
}
...
function editRevisionImages(regID, regrevID){
	modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'edit_drawing_register_revision_v1.php?regID='+regID+'&regrevID='+regrevID+'&name='+Math.random(), loadingImage, afterLoadRevision);
}
function afterLoadRevision(){
	var config = {
		...
		form: "addDrawingForm",// Form ID
		dragArea: "innerDivDrager",// Upload Area ID
		uploadUrl: "edit_drawing_register_revision_v1.php?antiqueID="+Math.random()// Server side upload url
	};
...
function addNewRegisterDocumentTransmital(existDTID){
	...
	modalPopup_gs(align, 10, 700, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'add_drawing_register_document_transmital_v1.php?&name='+Math.random()+'&attr1='+encodeURIComponent(attr1)+'&existDTID='+existDTID, loadingImage, afterLoadRegistrationDocumentTransmital, 1);
}
function afterLoadRegistrationDocumentTransmital(){
	var config = {
		...
		uploadUrl: "add_drawing_register_document_transmital_v1.php?antiqueID="+Math.random()// Server side upload url
	}
...
function donloadThis(fileName, fileType, projectID){
	$("#downloadFrame").attr("src", "download_document_register_files_v1.php?projectID="+projectID+"&fileType="+fileType+"&fileName="+fileName+"&antiqueId="+Math.random());
}
var searchTable = '<table cellpadding="0" cellspacing="0" border="0" class="display" id="drawingDefaultSearch" width="100%"><thead><tr><th>id</th><th width="70px">Document Number</th><th width="200px">Document Description</th><th>Rev.</th><th>Attribute 1</th><th>Attribute 2</th><th>Attribute 3</th><th>Tag</th><th>Status</th><th>Action</th></tr></thead><tbody><tr><td colspan="8" class="dataTables_empty">Loading data from server</td></tr></tbody></table>';
...
/*		var otpt = "";
	...
	oTable = $('#drawingDefaultSearch').dataTable( {
		...
		"bServerSide": true,
		"bRetrieve": true,
		"sAjaxSource": "drawing_register_data_table_search_v1.php?selectID="+encodeURIComponent(newDrawingID)+"&dispID="+encodeURIComponent(iVar)+"&req=search&attr1="+encodeURIComponent(currValue)+"&attr2="+encodeURIComponent(secAttrVal)+"&name="+Math.random(),
		"bStateSave": true,
...
function showMappingTable(newDrawingID, divRel, iVar){
	...
	modalPopup_gs(align, 20, 900, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'drawing_register_mapping_search_v1.php?&name='+Math.random()+'&attributeVal='+encodeURIComponent(attrVal), loadingImage, function(){loadSearchForm(newDrawingID, divRel, iVar);}, 2);
}
...
/*		var otpt = "";
	...
	var oTable = $('#drawingDefaultSearch').dataTable( {
		...
		"bServerSide": true,
		"bRetrieve": true,
		"sAjaxSource": "drawing_register_data_table_search_v1.php?selectID="+encodeURIComponent(hiddenNewDrawingID)+"&dispID="+encodeURIComponent(iVar)+"&req=search&attr1="+encodeURIComponent(drawingattribute1Search)+"&attr2="+encodeURIComponent(drawingattribute2Search)+"&searchKey="+encodeURIComponent(searchKeywordSearch)+"&name="+Math.random(),
		"bStateSave": true,
		/*"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
			for (var key in mappedDocArr) {
				if(aData[0] == mappedDocArr[key][0])
...
function selectUserNotification(userListHolder){	
	modalPopup_gs(align, top1, 690, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'document_transmittal_user_list.php?userListHolder='+userListHolder+'&name='+Math.random(), loadingImage, 2);
}
function submitSelectUserNotification(){
	showProgress();
	$.post("document_transmittal_user_list.php?antiqueID="+Math.random(), $('#addUserListForm').serialize()).done(function(data) {
		hideProgress();
		var jsonResult = JSON.parse(data);	
		if(jsonResult.status){
		...
		$('.saveToCropedImage').click(function(e) {
			...
			$.ajax({
			  ...
			  data: datastring,
				...
				$(".lc-drawing canvas:last-child").attr("ondrop", "drop(event)");
				$(".lc-drawing canvas:last-child").attr("ondragover", "allowDrop(event)");
...
function showMarkupImage(imgName, pdfID, projID){
	modalPopup_gs(align, 10, 850, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'drawing_previous_markups_image_view.php?imgName='+imgName+'&name='+Math.random(), loadingImage, function() {closePopup_gs(300);});
}
...
function mergeSelectedData() {	
	...
	var flagMerge = 0;
	$('.closeInspection').each(function(){
		console.log($(this).prop("checked"));
		if($(this).prop("checked")==true){
			flagMerge++;
			var newarr = $(this).attr('newvalue').split('###');	
			console.log(newarr);
			newArrayOfRev[newarr[3]+'**'+newarr[2]] = newarr[2]+'###'+newarr[0]; 			
			console.log(newArrayOfRev);
			if(checking==''){
				checking=newarr[4];
				RevArray.push(newarr[3]+'**'+newarr[2]);	
			}else{
				if(checking==newarr[4]){
					/*if(newarr[3]<minRev){
					minRev = newarr[3];
					console.log(minRev);
					}*/
				RevArray.push(newarr[3]+'**'+newarr[2]);		
					...
					jAlert('Document Numbers are not same. These files can not be merged.');
					flag = false;
					return false;
				}
			}
	...
	console.log(flagMerge);
	if(flagMerge ==1){
		jAlert('Please select another document to merge.');
		return false;
	}else if(flagMerge==0){
		jAlert('Please select documents to merge.');
		return false;
	}else{
	...
	if(flag==true){
		$.post("edit_drawing_register_v1.php", {revisionArr:newArrayOfRev, newRevArr : RevArray,  action:'merge'}).done(function(data) {
				$('.closeInspection').each(function(){
					//$(this).attr('checked','false');
					$('input:checkbox ').removeAttr('checked');
					//$('input:checkbox').prop('checked',false);
				});
				//$('.closeInspection').removeAttr('checked');
				jAlert('Successfully Merged');
				/*var attribute11 = $('#attr1').val();
				var attribute22 = $('#attr2').val();
				searchDrawPdfSecAttr(attribute11, attribute22);	*/
				RefreshTable();
		});
...
function donloadThisZip(revisionId, fileType, projectID, documentId){
	$("#downloadFrame").attr("src", "download_document_register_files_v1.php?projectID="+projectID+"&fileType="+fileType+"&revisionId="+revisionId+"&antiqueIdZip="+Math.random()+"&documentId="+documentId);
}
