...
module.exports = LiterallyCanvas = (function() {
  ...
  LiterallyCanvas.prototype.setZoom = function(scale) {
   ...
   // this.position.y = math.scalePositionScalar(this.position.y, this.canvas.height, oldScale, this.scale);
    this.position.x = math.scalePositionScalar(this.position.x, imgWidth, oldScale, this.scale);
    this.position.y = math.scalePositionScalar(this.position.y, imgHeight, oldScale, this.scale);
    this.keepPanInImageBounds();
    this.repaintAllLayers();
...
defineShape('Rectangle', {
  ...
  draw: function(ctx) {
	 /*Raj work*/
	var tmpColor;
	tmpColor = this.fillColor.replace("hsl", "hsla");
	tmpColor = tmpColor.replace("%)", "%, 0.5)");
	console.log(tmpColor);
    ctx.fillStyle = tmpColor; //this.fillColor;
	 /*Raj work*/	
...
defaultImageURLPrefix = 'markup_tool/img';
...
init = function(el, opts) {
  ...
  if (opts.secondaryColor == null) {
    opts.secondaryColor = 'transparent';//Raj '#fff';
  }
  ...
  if (!('tools' in opts)) {
    opts.tools = [tools.Pan, tools.Pencil, tools.Eraser, tools.Line, tools.Rectangle, tools.Ellipse, tools.Text, tools.Eyedropper];
  }
...
ColorWell = React.createClass({
  ...
  render: function() {
    ...
    return div({
      className: 'toolbar-button color-well-label '+this.props.label,
  ...
  renderPicker: function() {
    ...
    rows = [];
   //Raj if (this.props.colorName === 'background') {
