...
session_start();
	...
	$aColumns = array('issue_to_name', 'id');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "id";
	
	/* DB table to use */
	$sTable = "master_issue_to";
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	if ($sWhere == ""){
		$sWhere = "WHERE is_deleted=0 AND issue_to_name != '' ";
	}
	$sWhere = $sWhere. "";
	/*
	 * SQL queries
	...
	while ( $aRow = mysql_fetch_array( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			if ( $aColumns[$i] == "id" ){
				...
				$row[] = '<img class="action" src="images/edit_name.png" id="editIssueTo" title="Edit Issue To" onclick="editIssueToData('.$rowID.');" />&nbsp;&nbsp;<img class="action" src="images/users.png" id="users" title="Users" onclick="showIssueTo('.$rowID.');" /><!--&nbsp;&nbsp;<img class="action" src="images/add_name.png" id="addIssueToContact" title="Add New Issue To Contact" onclick="addNewIssueToContact('.$rowID.');" /-->&nbsp;&nbsp<img class="action" src="images/delete_name.png" id="editRevision" title="Delete Issue To"" onclick="deleteIssueTo('.$rowID.')" /><br />';				
