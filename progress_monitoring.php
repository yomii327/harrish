...
if($_SESSION['ww_builder_id']=="" && !empty($_SESSION['ww_company']['c_id'])){
	$builder_id = $_SESSION['ww_company']['c_id'];
}else{
	$builder_id = $_SESSION['ww_builder_id'];
}
...
//Update condition here
							if($updateFile == 1){
								while($rowP = mysql_fetch_assoc($resultTask)){
									$updateQry = "UPDATE progress_monitoring SET start_date = '".$startDate."', end_date = '".$endDate."', last_modified_date = NOW(), original_modified_date = NOW(), holding_point = '".$holdPointCurr."' WHERE progress_id = '".$rowP['progress_id']."'";
									$rs = mysql_query($updateQry);
									if(mysql_affected_rows() > 0){
										$updateCount++;
									}
									$update_issue = "UPDATE issued_to_for_progress_monitoring SET is_deleted = 1, last_modified_date = NOW() WHERE progress_id = '".$existProgressID."' AND project_id = '".$_SESSION['idp']."' AND is_deleted = 0";			
...
//Update condition here
							if($updateFile == 1){
								$existProgressID = "";
								while($rowP = mysql_fetch_assoc($resultTask)){
									$existProgressID = $rowP["progress_id"];
									$updateQry = "UPDATE progress_monitoring SET start_date = '".$startDate."', end_date = '".$endDate."', last_modified_date = NOW(), original_modified_date = NOW(), holding_point = '".$holdPointCurr."' WHERE progress_id = '".$rowP['progress_id']."'";
									$rs = mysql_query($updateQry);
									if(mysql_affected_rows() > 0){
										$updateCount++;
							...
							$inserQRY = "INSERT INTO progress_monitoring SET 
											...
											created_date = NOW(),
											last_modified_by = '".$builder_id."',
											last_modified_date = NOW(), original_modified_date = NOW(), holding_point = '".$holdPointCurr."'";
							mysql_query($inserQRY);
							$progress_id = mysql_insert_id();
							if($colIsuues[$h] != ''){
								$issueToArray = explode(';', $colIsuues[$h]);
...
//updating project record, to have no. of sub-locations get inserted
				$query = "UPDATE user_projects SET pr_num_sublocations = ".$num_sublocations.", last_modified_date = NOW() WHERE project_id = ".$_SESSION["idp"];
				mysql_query($query);
				...
				foreach($locData as $lData){
					...
					$query = 'UPDATE project_monitoring_locations SET location_tree_name = "'.$locNameTree.'", qrcode = "'.$locNameTree.'", last_modified_date = NOW() WHERE location_id = '.$lData['location_id'];
					mysql_query($query);
				}
//Message Part
...
$(document).ready(function() {
	...
	$('span.demo1').contextMenu('myMenu2', {
		bindings: {
			'add': function(t) {
				modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'location_tree_add_promon.php?location_id='+t.id, loadingImage);
			},
			'edit': function(t) {
				var parentId = $('#'+t.id).parent().parent().parent().get(0);
				modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'location_tree_edit_promon.php?location_id='+t.id+'&parent_id='+parentId.id, loadingImage);
			},
			'delete': function(t) {
			...
			'addTask': function(t) {
				modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'add_task_promon.php?sub_location_id='+t.id+'&uniqueId='+Math.random(), loadingImage, addDatePicker);
			},
			'qrCode': function(t) {
				console.log(t);
				modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'generate_qr_code.php?locationID='+t.id+'&tableName=project_monitoring_locations&uniqueId='+Math.random(), loadingImage);
			},
		}
	});
	...
	$('span.demo3').contextMenu('myMenu3', {
		bindings: {
			'viewTask': function(t) {
				modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'view_task_promon.php?progress_id='+t.id+'&uniqueId='+Math.random(), loadingImage);
			},
			'editTask': function(t) {
				modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'edit_task_promon.php?progress_id='+t.id+'&uniqueId='+Math.random(), loadingImage, addDatePicker);
			},
			...
			if(data != 'Duplicate task'){
				...
				$('span.demo1').contextMenu('myMenu2', {
					bindings: {
						...
						'edit': function(t) {
							modalPopup(align, topMargin, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'location_tree_edit_promon.php?location_id='+t.id, loadingImage);
						},
						'delete': function(t) {
							var r = jConfirm('Do you want to delete location ?, task added on this location also deleted !', null, function(r){
								if (r==true){
...
<div id="middle" style="padding-top:10px;">
	...
	<div id="rightCont" style="float:left;width:700px;">
		...
		<br clear="all" />
		<img title="Print QR Codes" style="margin-bottom:15px; margin-top:15px;cursor:pointer;" src="images/print-qr.png" id="printQRCodes">
		<div id="viewSwither">
...
function subLocate(val){
	AjaxShow("POST","ajaxFunctions.php?reqReport=Y&&type=subLocationPM && proID="+val,"ShowSubLocation1");
}
function subLocate_sub(val){
	AjaxShow("POST","ajaxFunctions.php?reqReport=Y&&type=subLocation_subPM && proID="+val,"ShowSubLocation2");
}
function resetIds(){}
function subLocate3QA(){}
function generateReport(){
	var locID = $('#location').val();
	var subLoc1ID = $('#subLocationPM').val();
	var subLoc2ID = $('#subLocation_subPM').val();
	var codePerPagePM = $('#codePerPagePM').val();
	if(locID==''){
		jAlert('Please select any location.');
		return false;
	}
	showProgress();
	$.post("pm_print_qr_codes.php", {locID:locID, subLoc1ID:subLoc1ID, subLoc2ID:subLoc2ID, uniqueID:Math.random(), codePerPage: codePerPagePM}).done(function(data) {
		hideProgress();
		var jsonResult = JSON.parse(data);	
		if(jsonResult.status){
			$('#outputDiv').html(jsonResult.html);	
		}else{
			//jAlert('Data updation failed, try again later');
			$('#outputDiv').html(jsonResult.msg);	
		}
	});
}
function removeThis(){
...
$('#printQRCodes').click(function(){
	...
	modalPopup(align, top1, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'pm_print_qr_codes.php?name='+Math.random(), loadingImage);
});


function saveImages(fileDownloaded){	window.location="download_image.php?file="+fileDownloaded;	}
</script>
<style>
