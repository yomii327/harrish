...
$location_query = "SELECT location_id, location_title, location_tree_name FROM project_monitoring_locations WHERE project_id = ".$_SESSION['idp']." AND location_parent_id = '".$_POST['cetid']."' AND is_deleted = '0' ORDER BY location_title";
...
if($isLocation > 0){
	...
	<ul  class="<?=$class?>" style="display:none" >
		<li id="li_<?=$locations['location_id']?>">
			<?php 
			$taskData = $obj->selQRYMultiple('progress_id, task, start_date, end_date', 'progress_monitoring', 'sub_location_id = '.$locations['location_id'].' AND is_deleted= 0 AND project_id = '.$_SESSION['idp'].' ORDER BY task');
			if($nextStatus){
