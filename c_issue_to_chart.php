...
//select * from issued_to_for_inspections order by issued_to_name
//create array of all inspection_ids
//select sum (open) close for each issued_to_name
//NOTE: Pending to set limit
...
$query = "SELECT count(*) as count, issued_to_name FROM `issued_to_for_inspections` as i, `project_inspections` as pi, user_projects as up where up.project_id=i.project_id and i.is_deleted=0 and up.is_deleted=0 and pi.is_deleted=0 and pi.inspection_id=i.inspection_id group by issued_to_name order by count desc limit $offset, $limit";
...
open , SUM( IF( i.inspection_status=  'Fixed', 1, 0 ) ) AS fixed, SUM( IF( i.inspection_status=  'Closed', 1, 0 ) ) AS closed, SUM( IF( i.inspection_status=  'Pending', 1, 0 ) ) AS pending FROM project_inspections d, issued_to_for_inspections i, user_projects up where i.issued_to_name='" . $issued_to_name ."' and i.inspection_id = d.inspection_id and up.project_id=i.project_id and d.project_id=up.project_id and i.is_deleted=0 and up.is_deleted=0 and d.is_deleted=0";
	...
	$issue_name[] =  '"<center>'.$issued_to_name.'<br/>' . $total . ' inspections<center>"';
    }
}
