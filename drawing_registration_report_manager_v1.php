...
?>
	<table width="100%" border="0">
		<tr>
			...
			<td style="padding-left:10px;">Company Name</td>
...
background-image: url(images/input_120.png);margin-left:0px;"  />
			...
			<td>
				 <!-- <img src="images/report_btn.png" onclick="runRerpor(<   ?=$_SESSION['idp']?>);" style="cursor:pointer; float:right; margin-left:10px;"  /> -->
				 <a class="green_small" href="javascript:void(0)" onclick="runRerpor(<?=$_SESSION['idp']?>);" style="cursor:pointer; float:right;"  alt="report" />Report</a>
			</td>
		</tr>
	</table>
...
/*	if(isset($_POST["revision"]) && !empty($_POST["revision"])){
	...
	$docData = array();
	$docData = $obj->selQRYMultiple('dr.id, dr.title, dr.created_by as addedBy, drr.pdf_name, dr.created_date, drr.created_by as revisedBy, dr.is_approved, drr.revision_number, drr.created_date AS revisionAdded', 'drawing_register_revision_module_one AS drr, drawing_register_module_one AS dr', 'drr.is_deleted = 0 AND dr.is_deleted = 0 AND  drr.drawing_register_id = dr.id AND dr.project_id = '.$_REQUEST['projID'].' AND drr.project_id = '.$_REQUEST['projID'].$sWhere);
