...
{
	$project_id_new=$_SESSION['issu_to_pro']=$_POST['pmId'];	
	
	$query = "SELECT count(*) as count, issued_to_name FROM `issued_to_for_inspections` as i, user_projects as up where 					up.user_id=$builder_id and up.project_id=$project_id_new and i.project_id=$project_id_new and i.is_deleted=0 and up.is_deleted=0 group by issued_to_name order by count desc limit $offset, $limit";
...
$q="select $field from $table where $where GROUP BY $group";
	...
	while($q1=mysql_fetch_array($q)){
		?>
		<option <?php if(isset($_SESSION['issu_to_pro'])) { if($_SESSION['issu_to_pro'] == $q1[0]){?> selected="selected" <?php }} ?> value="<?php echo $q1[0]; ?>"><?php echo $q1[1]; ?></option>
	<?php
    }
}
?>
...
Select Project
<select name="projName" id="projName"  class="select_box" onChange="changeProID(this.value);" style="background-image:url(images/selectSpl.png);">
...
<div id="chart2" style="width:875px; height:320px;text-align:center;"></div>
</body>
</html>