...
//	$aColumns = array('issue_to_name', 'company_name', 'issue_to_phone', 'issue_to_email', 'tag','issue_to_id');
	$aColumns = array('checklist_name', 'id');
		
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "id";
	
	/* DB table to use */
	$sTable = "check_list_items_project";
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	$sWhere = $sWhere. "GROUP BY checklist_name";
	/*
	 * SQL queries
	 * Get data to display
	 */
	...
	while ( $aRow = mysql_fetch_array( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			if ( $aColumns[$i] == "id" ){
				...
				$row[] = '&nbsp;&nbsp;&nbsp;&nbsp <img class="action" src="images/edit_right.png" id="editChecklist" title="Edit Checklist" onclick="editProjectChecklistData('.$rowID.');" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!-- <img class="action" src="images/task.png" id="users" title="View Task" onclick="showtask('.$rowID.');" /><!--&nbsp;&nbsp;<img class="action" src="images/add_name.png" id="addIssueToContact" title="Add New Issue To Contact" onclick="addNewIssueToContact('.$rowID.');" /-->&nbsp;&nbsp;&nbsp;<img class="action" src="images/delete.png" id="editRevision" title="Delete Checkist"" onclick="deleteProjectChecklist('.$rowID.')" />';
