...
//Default folder creatation end here
	...
	$drawingRegData = $obj->selQRYMultiple('drr.id, dr.number, dr.title, dr.pdf_name', 'drawing_register AS dr, drawing_register_revision AS drr', 'drr.id IN ('.$_REQUEST['fileIds'].') AND dr.id = drr.drawing_register_id AND dr.is_deleted = 0 AND drr.is_deleted = 0');

	foreach($drawingRegData as $drRegData){
		$fileExt = end(explode('.', $drRegData['pdf_name']));
		
		$newFileName = substr(str_replace( array('\\', '/', '"', ':', '*', '?', '<', '>', '|', '\r\n', '\\r\\n', '\n', '\\n', "\r\n", "\\r\\n", "\n", "\\n"), ' ', $drRegData['number'].'-'.$drRegData['title']), 0, 150);
...
//Default folder creatation start here
	...
	$drawingRegData = $obj->selQRYMultiple('drr.id, dr.number, dr.title, dr.pdf_name, dr.dwg_name, dr.img_name', 'drawing_register AS dr, drawing_register_revision AS drr', 'drr.id = '.$_REQUEST['fileName'].' AND dr.id = drr.drawing_register_id AND dr.is_deleted = 0 AND drr.is_deleted = 0');
	$fileExt = '';	
	...
	foreach($drawingRegData as $drRegData){
		...
		switch($_REQUEST['fileType']){
			...
			case 'IMG':
				$fileExt = end(explode('.', $drRegData['img_name']));
