...
if(isset($_POST['uniqueId'])){
	...
	$locData = $obj->selQRYMultiple('location_title', 'qa_task_locations', 'project_id = '.$_SESSION['idp'].' AND is_deleted = 0 AND location_parent_id = '.$parentID);
	$locCheckArr = array();
	if(!empty($locData)){
	...
	if(!in_array($location, $locCheckArr)){	
		$q="UPDATE qa_task_locations SET
		...
		$res = mysql_query($q);
		$locNameTree = $obj->qa_sublocationParent($locationId, ' > ');
		$query = 'UPDATE qa_task_locations SET location_tree_name = "'.$locNameTree.'", last_modified_date = NOW(), last_modified_by = '.$_SESSION['ww_builder_id'].' WHERE location_id = '.$locationId;
		mysql_query($query);
...
if(isset($_GET['location_id'])){
	if(strpos($_GET['location_id'], 'Id_')){
		...
		$q="SELECT location_id, location_parent_id, location_title FROM qa_task_locations WHERE location_id = '".$_GET['location_id']."' AND is_deleted = 0";
		$res = mysql_query($q);
		...
		<fieldset class="roundCorner">
			...
			<table width="100%" border="0">
				...
				<tr>
					<td align="center" colspan="2" style="padding-top:20px;"><input type="button" name="submit" id="submit" value="Submit" onclick="editLocation()"  />
                    
                    <input type="button" id="s1" onclick="closePopup(300)" value="Cancel">
                    </td>
				</tr>
			</table>
		</fieldset>
