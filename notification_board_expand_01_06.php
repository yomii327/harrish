...
if($overdue=$obj->db_fetch_assoc($noti_record)){
  ...
  $noti_one_day="SELECT count(*) as due_one FROM `issued_to_for_inspections` as i, user_projects as up where up.project_id=$project_id_new and i.project_id=$project_id_new and i.is_deleted=0 and up.is_deleted=0 and i.is_deleted=0 and user_id=$builder_id and i.inspection_status !='Closed' and inspection_fixed_by_date >= CURDATE() - INTERVAL 1 DAY and inspection_fixed_by_date <=CURDATE() and i.inspection_fixed_by_date!='0000-00-00'";
