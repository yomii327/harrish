...
if (isset($_REQUEST["name"])){
	...
	if($noInspection > 0){
		...
		$pdf->Image($logo_proj, 115, 1, 'png', -130);
		$pdf->Ln(14);
		
		$pdf->SetFillColor(0, 0, 0);
		$pdf->SetTextColor(252, 252, 252);
		$pdf->SetFont('Helvetica', 'B', 18);
		$pdf->Cell(190, 10, ' MEETING MINUTES', '', '', '', true);		
		...
		*/
		$pdf->SetFont('Helvetica', 'B', 11);
		$pdf->Cell(100, 10, $meetingData[0]['description']);		
		$pdf->Ln(5);
...
//Title String Header
		$file_name = 'meeting_minutes_report_'.microtime().'.pdf';
