...
if(isset($_REQUEST['uniqueId'])){
	...
	if(!empty($_REQUEST['projName'])){
		...
		//To be changed with release
		//$pr_num_sublocations = 1;
		...
		if(!empty($querySubLoc)){
			...
			$pagecount = 0;
                        $tot_loc_count = 1;
			$html_location[$pagecount] = '<tr><td style="width:290px;height:50px;">Status</td><td style="width:60px;padding:2px;">'.date('d/M/Y').'</td>';
			$html_sub_location[$pagecount] = '<tr><td style="width:290px;height:50px;"><strong>TASK</strong></td><td style="width:60px;height:50px;">';
			if ($pr_num_sublocations == "2")
			{
				$html_sub_sub_location[$pagecount] = '<tr><td style="width:290px;height:50px;"><strong>TASK</strong></td><td style="width:60px">';
        			$html_sub_location[$pagecount] = '<tr><td style="width:290px;height:50px;">&nbsp;</td><td style="width:60px">';
			}
...
//Issue to for progress report
			...
			foreach($queryTaskData as $taskData){
				if ($issue_flag){
					$html_tds[$vpagecount] [$hpagecount] .= '<td style="height:73px;padding-left:2px;"><div >'.$taskData['task'].'</div>';
				...
				if($postion != $i){
					...
					for ($j=$tmp; $j<$postion;$j++){
						...
						$html_tds [$vpagecount] [$hpagecount] .= "<td style='height:73px;width:100px'><div>&nbsp;</div><div style='border-bottom:1px solid black'>&nbsp;</div><div>&nbsp;</div></td>";
						$i++;
					}
				}
				if (intval($i/$col)==($i/$col))
				...
				$html_tds [$vpagecount] [$hpagecount] .= "<td style='height:73px;width:100px'>
...
foreach($html_tds as $html_h){
    for ($i=0;$i<count($html_h);$i++){
		...
		if($page==1){	$html .= $headerHtml;	}else{	$html .= '<div style="height:121px;">&nbsp;</div>'; }
...
$total_page = ($hpagecount+1)*($vpagecount+1);
if ($page_no == $total_page)
{
	for ($i=1;$i<=$total_page; $i++)
	{
		$zipFileArray[] = '../report_pdf/'.$owner_id.'/wallChart_part'.$i.'.pdf';
	}
	$zipName = 'wall_chart_pdf'.microtime().'.zip';
	
	if($obj->create_zip($zipFileArray, '../report_pdf/'.$owner_id.'/'.$zipName)){
		echo '<br clear="all" /><div style="margin-left:10px;">Wall Chart Report Generated <a onClick="closePopUp();" href="report_pdf/'.$owner_id.'/'.$zipName.'" target="_blank" class="download_btn">&nbsp;</a></div>';
	}else{
