...
if (isset($_REQUEST["name"])){
	...
	$docData = array();
	$attribute1Arr = array('"General"', '"Architectural"', '"Structure"', '"Services"', '"Concrete & PT"', '"Civil"', '"ESD"', '"Lifts"', '"Lighting"', '"Tenancy Fitout"', '"Penthouse Architecture"', '"Landscaping"');
	...
	#	$docData = $obj->selQRYMultiple('dr.id, dr.number, dr.title, dr.pdf_name, dr.created_date, dr.is_approved, drr.revision_number, drr.created_date AS revisionAdded, dr.attribute1, dr.attribute2, dr.status, drr.last_modified_by'.$sect, 'drawing_register_revision AS drr, drawing_register AS dr', 'drr.is_deleted = 0 AND dr.is_deleted = 0 AND drr.drawing_register_id = dr.id AND dr.attribute1 IN ('.join(',', $attribute1Arr).') AND dr.is_document_transmittal = 0 AND dr.project_id = '.$_REQUEST['projID'].' AND dr.project_id = '.$_REQUEST['projID'].' '.$groupBy.' '.$orderBy);
if(isset($_GET['spectialCon']) && $_GET['spectialCon'] == 'Y'){
	$docData = $obj->selQRYMultiple('dr.id, dr.number, dr.title, dr.pdf_name, dr.created_date, dr.is_approved, drr.revision_number, drr.created_date AS revisionAdded, dr.attribute1, dr.attribute2, dr.status, drr.last_modified_by'.$sect, 'drawing_register_revision AS drr, drawing_register AS dr', 'drr.is_deleted = 0 AND dr.is_deleted = 0 AND drr.drawing_register_id = dr.id AND dr.attribute1 IN ('.join(',', $attribute1Arr).') AND dr.is_document_transmittal = 0 AND dr.project_id = '.$_REQUEST['projID'].' AND dr.project_id = '.$_REQUEST['projID'].' '.$groupBy.' '.$orderBy);
}else{
/*	echo 'SELECT temp.*, drawing_register_revision.last_modified_by from
								(SELECT
										dr.id,
										dr.number,
										dr.title,
										...
										dr.is_approved,
										drr.revision_number,
										drr.created_date AS revisionAdded,
										dr.attribute1,
										dr.attribute2,
										dr.status '.$sect.' 
									FROM
										drawing_register_revision AS drr,
										drawing_register AS dr
									WHERE
										drr.is_deleted = 0 AND
										dr.is_deleted = 0 AND
										drr.drawing_register_id = dr.id AND
										dr.attribute1 IN ('.join(',', $attribute1Arr).') AND
										dr.is_document_transmittal = 0 AND
										dr.project_id = '.$_REQUEST['projID'].' AND
										dr.project_id = '.$_REQUEST['projID'].'
									'.$groupBy.'
									'.$orderBy.') AS temp JOIN drawing_register_revision
									ON temp.revID = drawing_register_revision.id';die;*/
	$docData = $obj->getRecordByQuery('SELECT temp.*, drawing_register_revision.last_modified_by from
								(SELECT
										dr.id,
										dr.number,	
										dr.title,
									...
									'.$groupBy.'
									'.$orderBy.') AS temp JOIN drawing_register_revision
									ON temp.revID = drawing_register_revision.id');
}
