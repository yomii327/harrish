...
//Generate subloction id array
		...
		if($locationCount >= 17){
			...
			$bHeight = 2;
			$best_height=10;
			$widthArray = array(19, 12, 15);//status date locations
		}
			...
			$bHeight = 3;
			$best_height=14;
			$widthArray = array(19, 13, 15.8);//status date locations
		}
			...
			$bHeight = 3;
			$best_height=14;
			$widthArray = array(23, 13, 16.5);//status date locations
		}
			...
			$bHeight = 2.7;
			$best_height=10;
			$widthArray = array(26, 16, 17);//status date locations
		}
			...
			$bHeight = 2.7;
			$best_height=10;
			$widthArray = array(29, 16, 18);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(32, 16, 19.5);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(33, 16, 21);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(33, 16, 23);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(37, 16, 25);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(42, 16, 28);//status date locations
		}
			...
			$bHeight = 3.0;
			$best_height=15;
			$widthArray = array(47, 16, 31);//status date locations
		}
		...
		class PDF extends PDF_MC_Table{
			...
			function Header(){
				if($this->PageNo()!=1){
					...
					$locHeader = array('Status', date('d/M/Y'));
			...
			function header_width($widthArr, $lupCount){
				$arr = array($widthArr[0], $widthArr[1]);
				for($i=0; $i<$lupCount; $i++){
					array_push($arr, $widthArr[2]);
				}
				return $arr;
			}
		}
...
//Count location name for headers
		...
		if(!empty($querySubLoc)){
			...
			$hLocVal = array_merge($hLocVal, array('Status', date('d/M/Y')));
	...
	//echo '<pre>';print_r($html_sub_location1[1]);die;
		...
		for ($ik=0; $ik<=$pagecount; $ik++){
			...
			foreach($queryTask as $tasks){
				...
				array_push($hTaskVal, $hpoint_prefix."Start\nFinish\nStatus");
				for ($j=0; $j < count($sub_location_position[$ik]); $j++){
					...
					array_push($hTaskVal, $hpoint_prefix."STATUS##" .$value[3] . "~~" . date('d-M-Y', strtotime($value[0])). "\n" .date('d-M-Y', strtotime($value[1])). "\n" .$value[2]);
				}
