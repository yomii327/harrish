...
if($sect=='apply_now'){
	$_SESSION['post_array']=$_POST;
		...
		/*if(isset($_FILES['c_logo']['name']) && !empty($_FILES['c_logo']['name']))
		{
			...
			{
				$path=$_SERVER['DOCUMENT_ROOT'].'/wiseworkers/company_logo/'; // CSV Path
	...
	if($fname=='' || $compname=='' || $email=='' || $mobile=='' || $username=='' || $password==''){
		...
		if($obj->db_num_rows($obj->db_query($q)) > 0){
			...
			if($obj->db_num_rows($obj->db_query($q)) > 0){
				...
				$date=Date('Y-m-d H:i:s');
				$q = "INSERT INTO ".BUILDERS." (fk_c_id,user_verifycode,active,user_name,user_password,user_plainpassword,user_fullname,company_name,user_email,user_phone_no,created_date) 
					  VALUES('".$_SESSION['ww_c_id']."','$verifyCode','1','$username','$pswdmd5','$password','$fname','$compname','$email','$mobile','$date')";
					 // echo $q; die;
				$obj->db_query($q);
	...
	if($fname=='' || $compname=='' || $email=='' || $mobile=='' || $username==''){		
		...
		if($obj->db_num_rows($obj->db_query("SELECT * FROM ".BUILDERS." WHERE builder_user_name='$username' AND id!='".$_SESSION['ww_builder_id']."'")) > 0){
			...
			$q="UPDATE ".BUILDERS." SET user_name='$username',user_fullname='$fname',company_name='$compname',user_email='$email',user_phone_no='$mobile'
				$pswd WHERE user_id='".$_SESSION['ww_builder_id']."'";
...
elseif($sect=='add_project'){
	...
	if($protype=='' || $name=='' || $line1=='' || $suburb=='' || $state=='' || $postcode=='' || $country==''){
		...
		$q = "INSERT INTO ".PROJECTS." (builder_id,project_name,project_type,project_address_line1,project_address_line2,project_suburb,project_state,post_code,project_country) VALUES('$builder_id','$name','$protype','$line1',
		'$line2','$suburb','$state','$postcode','$country')";
		$obj->db_query($q);
		...
		$q = "UPDATE ".PROJECTS." SET pro_code='$pro_code' WHERE id='$lastProId'";
		$obj->db_query($q);		
...
elseif($sect=='edit_project'){
	...
	if(isset($_POST['edit'])){
		...
		if($protype=='' || $name=='' || $line1=='' || $suburb=='' || $state=='' || $postcode=='' || $country==''){
			...
			$q = "UPDATE ".PROJECTS." SET project_name='$name',project_type='$protype',project_address_line1='$line1',project_address_line2='$line2',project_suburb='$suburb',project_state='$state',project_postcode='$postcode',project_country='$country' 
...
elseif($sect=='forgot_password'){
	...
	if($email==''){
		...
		//$result=1;
		$_SESSION['error']['message']="Invalide email format.";
		header("location:pms.php?sect=forgot_password&type=$type");		
	}else{
		...
		if($obj->db_num_rows($obj->db_query($q)) > 0){
			...
			$obj->send_mail($detail);
			//$result=2;	sleep(1);
			$_SESSION['error']['message']="Login info has sent to ".$email;
			$_SESSION['error']['type']="success_r";
			header("location:pms.php?sect=forgot_password&type=$type");
		}else{
			//$result=3;
			$_SESSION['error']['message']="Email id is not registered!";
		header("location:pms.php?sect=forgot_password&type=$type");
		}		
	}
}
