...
//issue to data fetch here
	...
	$toEmailIDS['reportType'] = 'Summary Report with Cost Impact';
...
$html = '<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		if($offset == 0){
			$html .= '
				<tr>
					<td width="40%" style="font-size:14px;"><u><b>Summary Report with Cost Impact</b></u></td>
					<td>&nbsp;</td>
				</tr>
				<tr>
			...
			if(!empty($_REQUEST['subLocation']) && !empty($_REQUEST['sub_subLocation'])){
				...
				<td width="200" style="font-size:11px;" valign="top">'.$subSubLocationSearchName.'</td>';$jk++;
			}else{
				if(!empty($_REQUEST['location']) && !empty($_REQUEST['subLocation'])){
					$html .= '<td width="170" style="font-size:11px;" valign="top"><b>Location&nbsp;Name: </b></td>
					<td width="200" style="font-size:11px;" valign="top">'.$locationSearchName.'</td>';$jk++; if($jk%2 !=0){$html .= '';}if($jk%2 ==0){$html .= '</tr><tr>';}
			...
			if(!empty($_REQUEST['subLocation']) && !empty($_REQUEST['sub_subLocation'])){
				...
				<td width="200" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['sub_subLocation'], 'location_title').'</td>';$jk++;
			}else{
				if(!empty($_REQUEST['location']) && !empty($_REQUEST['subLocation'])){
					$html .= '<td width="170" style="font-size:11px;"><b>Location&nbsp;Name: </b></td>
					<td width="200" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['location'], 'location_title').'</td>';$jk++; if($jk%2 !=0){$html .= '';}if($jk%2 ==0){$html .= '</tr><tr>';}
	...
	$html .= '<td width="4%" style="font-size:10px;" align="center"><strong>Inspected By</strong></td>
			...
			<td width="4%" style="font-size:10px;" align="center"><strong>Cost Attribute</strong></td>
			<td width="3%" style="font-size:10px;" align="center"><strong>Cost Impact</strong></td>
			<td width="3%" style="font-size:10px;" align="center"><strong>Cost Impact Price</strong></td>
			<!--<td width="3%" style="font-size:10px;" align="center"><strong>Time Impact</strong></td>-->';
		$html .= '</tr>';
		$i=0;$j=1;
		$grandTotal = 0;
		while($fi=mysql_fetch_assoc($ri)){$i++;
			...
			$issueToData = $obj->selQRYMultiple('issued_to_name, inspection_fixed_by_date, cost_attribute, inspection_status, cost_impact_type, cost_impact_price, time_impact', 'issued_to_for_inspections', 'inspection_id = '.$fi['InspectionId'] . ' and is_deleted=0 ' . $where .' group by issued_to_name');

			$issueToData_issueToName = ""; $issueToData_fixedByDate= ""; $issueToData_status= "";$issueToData_costAttribute='';
			$issueToData_cost_impact = ""; $issueToData_cost_impact_price = "";$totalPrice = 0;$timeImpact='';
			if(!empty($issueToData)){
				foreach($issueToData as $issueData){
					...
					if($timeImpact == ''){
						$timeImpact = stripslashes(str_replace(" ", "&nbsp;", $issueData['time_impact']));
					}else{
						$timeImpact .= ' > '.stripslashes(str_replace(" ", "&nbsp;", $issueData['time_impact']));
					}
				}
	...
	$html .= '<td width="5%"><font style="font-size:10px;">'.stripslashes(wordwrap($fi["InspectedBy"], 15, "<br />", true)).'</font></td>
			...
			<td width="4%" align="center"><font style="font-size:10px;">'.$issueToData_costAttribute.'</font></td>
			<td width="4%" align="center"><font style="font-size:10px;">'.$issueToData_cost_impact.'</font></td>
			<td width="4%" align="center"><font style="font-size:10px;">'.$issueToData_cost_impact_price.'</font></td>
			<!--<td width="4%" align="center"><font style="font-size:10px;">'.$timeImpact.'</font></td>-->';
			$html .='</tr>';
	...
	$html .= '<tr><td colspan="11"><h3>Cost Impact Price Total</h3></td><td align="center">'.number_format($grandTotal, 2, '.', '').'</td><td>&nbsp;</td></tr>
