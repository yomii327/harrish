...
session_start();
	...
	$aColumns = array('issue_to_name', 'company_name', 'issue_to_phone', 'issue_to_email', 'tag','is_default','contact_id');
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ ){
		if ( $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' ){
			if ( $sWhere == "" ){
				$sWhere = "WHERE is_deleted=0 AND master_issue_id = '".trim($_REQUEST['issueToId'])."' ";
			}else{
				$sWhere .= " AND ";
			}
