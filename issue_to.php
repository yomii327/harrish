				...
				for($i=1;$i<$num;$i++) //read second line beacuse first line cover headings
				{
					if(!empty($fieldarray[$i][1]))
					{
						...
						if($row_data > 0)
						{
							...
							$record=count($fieldarray[$i][0]);//keep Duplicate Record list.
							if($record>0)
							{
								$count=$count+1;
							}
						...
						else
						{
							...
							@$insert = "INSERT INTO inspection_issue_to SET
											issue_to_name = '".$fieldarray[$i][1]."',
											company_name = '".$fieldarray[$i][0]."',
											issue_to_phone = '".$fieldarray[$i][2]."',
											issue_to_email = '".$fieldarray[$i][3]."',
...
<div style="width:722px; height:50px; float:left; margin-top:5px;">
	<form method="post" name="csvIssueto" id="csvIssueto" enctype="multipart/form-data">
		<table width="690px" border="0" cellspacing="0" cellpadding="3">
			<tr>
				<td colspan="4" align="left"><a href="/csv/issueto.csv" style="text-decoration:none;color:#FFF;"><strong style="font-size:16px;">Click here to download CSV template</strong></a></td>
			</tr>   
			<tr>
				...
				<td width="120px;" height="50px"><input type="image" src="images/import_csv_btn.png"  name="location_csv" id="location_csv" value="Import CSV" style="margin-left:15px;" /></td>
			</tr>
		</table>
	</form>
<br clear="all" />
...
<div class="big_container" style="width:722px;float:left;margin-left:10px;" ><?php include'issueto_csv_table.php';?></div>
</div>
