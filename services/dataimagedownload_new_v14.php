...
//Get Accepted Ids	
			...
			if(!empty($noofFiles)){
				...
				foreach($projectId as $project){
					$imageCountQry = $db->selQRY('count(*) AS imageCount', 'inspection_graphics', ' project_id = "'.$project['project_id'].'" AND graphic_type = "images" AND is_deleted = 0 AND inspection_id IN ('.$conD[$project["project_id"]].') ORDER BY inspection_id');
				...
				$imageName = $db->selQRYMultiple('graphic_name, last_modified_date', 'inspection_graphics', 'inspection_id IN ('.$insSelect.') AND graphic_type = "images"  AND is_deleted = 0 ORDER BY inspection_id');
				...
				if($db->emptyDirectory($zipSource)){
					...
					copy(EXPORTFILEPATH.'/'.$globalId.'/photo.zip', EXPORTFILEPATH.'/'.$globalId.'/image_'.$zipFileName.'.zip');
...
//new Project accepted condition GS Dated : 22-03-2013
			...
			if(!empty($noofFiles)){
				$imageCountQry = $db->selQRY('count(*) AS imageCount', 'inspection_graphics', 'project_id IN ('.$projId.') AND inspection_id IN ('.$inspSelect.') AND inspection_id NOT IN ('.$inspDeSelect.') AND graphic_type = "images" AND is_deleted = 0');
				$imageCount = $imageCountQry['imageCount'];
				if($imageCount > 0){
					$output = array(
						'status' => true,
				...
				$imageName = $db->selQRYMultiple('graphic_name, last_modified_date', 'inspection_graphics', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND inspection_id NOT IN ('.$inspDeSelect.') AND graphic_type = "images" AND is_deleted = 0 ORDER BY inspection_id ASC LIMIT '.$startIndex.', '. ($imageLimit + 1));
				$folder = opendir(IMAGESOURCEPATH);
				$pic_types = array("jpg", "jpeg", "gif", "png");
				$index = array();
				$count = 0;
...
//Location Condition here
		...
		if($statsRange[$project['project_id']] == "'ALL'" && $dateRange[$project['project_id']] == 100000){
			...
			if (!empty($issuedToNameProjectId[$project['project_id']])){
				...
				$rsIssuetoInspections = mysql_query("SELECT inspection_id FROM inspection_graphics WHERE project_id = '".$project['project_id']."' AND graphic_type = 'images' AND is_deleted=0 AND inspection_id IN (".join (",", $inspId).") ORDER BY inspection_id");
				$inspId = array();		
				if(mysql_num_rows($rsIssuetoInspections) > 0){
					while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
						$inspId[] = $db->dataFilter($rowIssuetoInspections['inspection_id']);
