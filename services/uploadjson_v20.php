...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										$query = "INSERT INTO ".$tableNamePinspections." SET `inspection_id` = '".$jDataPinspections['inspection_id']."',
												...
												`inspection_latitude` = '".$jDataPinspections['inspection_latitude']."',
												`inspection_longitude` = '".$jDataPinspections['inspection_longitude']."',
										...
										if(mysql_query($query)){
											...
											#Add inspection_id into cron inspection arr.
											$cornInspectionArr[$jDataPinspections['inspection_id']]['inspection_id'] = $jDataPinspections['inspection_id'];
											$cornInspectionArr[$jDataPinspections['inspection_id']]['project_id'] = $jDataPinspections['project_id'];
										}else{
											$rowArray = array($sync_url, $zipName.'.zip', mysql_error(), $deviceType, $userId, "Now()", $userId, "Now()");
										}
									}else{
...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										...
										if(mysql_query($query)){
											...
											#Add issued_to_name cron inspection arr.
											$cornInspectionArr[$jDataInspection['inspection_id']]['issued_to_name'][] = addslashes($jDataInspection['issued_to_name']);
										}else{
											$rowArray = array($sync_url, $zipName.'.zip', mysql_error(), $deviceType, $userId, "Now()", $userId, "Now()");
										}
									}else{
...
//ProgressMonitoring Checklist Data
				...
				#Insert inspection data into cron table(cron_for_pending_emails).
				if(isset($cornInspectionArr) && !empty($cornInspectionArr)){
					foreach($cornInspectionArr as $cRows){
						$cronQuery = 'INSERT INTO cron_for_pending_emails SET
							primary_key = '. $cRows['inspection_id'] .',
							project_id = '. $cRows['project_id'] .',
							section_name = "project_inspections",
							data = \''. serialize($cRows['issued_to_name']) .'\',
							is_new_record = 1,
							is_done = 0,
							created_by = 0,
							created_date = NOW(),
