...
//Only Selected Projects Data comes		
			...
			if(!empty($noofFiles)){
				...
				if($db->emptyDirectory($zipSource)){
					...
					$zipFileName = $db->updateExportTable_new(EXPORTFILEPATH.'/'.$globalId.'/photo/', $_REQUEST['userId'], $deviceType, 'images');
...
#			$projectId = $db->getRecordByKey('user_projects', 'user_id', $userId, 'project_id');
			...
			if(!empty($noofFiles)){
				...
				$imageName = $db->selQRYMultiple('graphic_name, last_modified_date', 'inspection_graphics', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND graphic_type = "images"  AND last_modified_date >= "'.$lastModifiedDate.'" AND is_deleted = 0 ORDER BY inspection_id ASC LIMIT '.$startIndex.', '.$imageLimit);
				$folder = opendir(IMAGESOURCEPATH);
				$pic_types = array("jpg", "jpeg", "gif", "png");
				$index = array();
				foreach($imageName as $imgName){
