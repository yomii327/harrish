...
#			$curTime = $db->selQRY("MAX(created_date) as curTime", "importData", "userid = ".$userId." AND device = '".$deviceType."'");

#			$subQuery = "SELECT inspection_id FROM project_inspections WHERE last_modified_by = ".$userId." AND resource_type = '".$deviceType."' AND last_modified_date BETWEEN ('".$curTime['curTime']."' - INTERVAL 1 MINUTE) AND '".$curTime['curTime']."'";
			$inspArray = array();
			foreach($projectId as $project){
				$inspList4Role = '';
				...
				$rsProjectInspections = mysql_query("SELECT ".$selProjectInspections." FROM project_inspections WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0".$roleProjectId[$project['project_id']]);
