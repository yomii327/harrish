...
if(isset($_REQUEST['non_conf_images'])){
	...
	@mkdir(EXPORTFILEPATH.'/conform/image/', 0777);
		...
		if(empty($lastModifiedDate)){
			...
			if($noofFiles == 'Y'){
				...
				foreach($imageName as $imgName){
					if($imgName['qa_graphic_type'] == 'image'){
						if(file_exists(IMAGESOURCEPATH.'/'.$imgName['qa_graphic_name'])){
							copy(IMAGESOURCEPATH.'/'.$imgName['qa_graphic_name'], EXPORTFILEPATH.'/conform/image/'.$imgName['qa_graphic_name']);
