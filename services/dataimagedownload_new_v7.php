...
//Only Selected Projects Data comes		
			...
			foreach($projectId as $project){
				...
				if($statsRange[$project['project_id']] == "'ALL'" && $dateRange[$project['project_id']] == 100000){
					$rsIssuetoInspections = mysql_query("SELECT inspection_id FROM inspection_graphics WHERE project_id = '".$project['project_id']."' AND graphic_type = 'images' AND is_deleted=0 ORDER BY inspection_id");
					$inspId = array();		
			...
			if(!empty($noofFiles)){
				...
				foreach($projectId as $project){
					...
					$imageCountQry = $db->selQRY('count(*) AS imageCount', 'inspection_graphics', ' project_id = "'.$project['project_id'].'" AND '.$whereCon.'  graphic_type = "images" AND is_deleted = 0 ORDER BY inspection_id');
