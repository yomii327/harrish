...
//Data for Issued to for Inspections Table			
			...
			if(count($iPadQueryIssuetoInspections) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "progress_monitoring_check_list", 'global_id, project_id, progress_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted', $iPadQueryIssuetoInspections);
			}		
			...
			foreach($projectId as $project){
				if (!isset($new_project [$project['project_id']]))
				{
					...
					if(mysql_num_rows($rsIssuetoInspections) > 0){
						...
						$db->createFile('progress_monitoring_check_list_update.txt', join('', $iPadQueryIssuetoInspections), EXPORTFILEPATH.'/download/');//Write File Here 
					}
					#echo $iPadQueryIssuetoInspections;
				}
			}
