...
//Files crate Here
		if(empty($lastModifiedDate)){//Modified Date not comes form device
			include('first_sync_v38.php');
		}else{//Modified Date comes form device
			include('sub_sequent_sync_v38.php');
		}
...
//Code for Download the zip file
		...
		echo 'Dir or Folder "../sync/Export" not exists.';
...
function getUserRoleData($userID){
	...
	$user_roles = $db->selQRYMultiple('user_projects.user_role, user_projects.project_id, user_projects.issued_to', 'user_projects JOIN projects ON projects.project_id = user_projects.project_id', 'user_projects.user_id = "'.$userID.'" AND user_projects.is_deleted = 0 AND projects.is_deleted = 0');
	foreach($user_roles as $user_role){
		if($user_role['user_role'] != 'All Defect' && $user_role['user_role'] != 'Sub Contractor'){
			...
			if ($user_role['user_role'] == 'Sub Contractor'){
				...
				$issuedToNameProjectId[$user_role['project_id']] =  ' and (isi.issued_to_name="' . $user_role["issued_to"] . '" OR isi.issued_to_name LIKE "' . $user_role["issued_to"] . ' (%")';
			}
		}
...
function createUserProjectsFile($table, $colString, $projectIDs, $global_id){
	...
	//$rs = mysql_query("SELECT ".$selStr." FROM ".$table." WHERE project_id IN (".$projectIDs.") AND created_date >= '".$lastModifiedDate."' AND is_deleted = 0");
	$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." AS up LEFT JOIN user AS u ON u.user_id = up.user_id WHERE project_id IN (".$projectIDs.") AND up.is_deleted = 0");
	...
	if(mysql_num_rows($rs) > 0){
		while($row = mysql_fetch_assoc($rs)){
			...
			for($i=0; $i<$loopCount; $i++){
				if($valStr == ''){
					$colName = stristr($colArray[$i], '.');
					$colName = substr($colName, 1, strlen($colName));
					$valStr = "\"".$db->dataFilter($row[$colName])."\"";
				}else{
					$colName = stristr($colArray[$i], '.');
					$colName = substr($colName, 1, strlen($colName));
					$valStr .= ", \"".$db->dataFilter($row[$colName])."\"";
		...
		if($changeStatus[$project['project_id']] == 1){//Last modified date condition not affected here like first sync
			...
			if($statsRange[$project['project_id']] == "'ALL'" && $dateRange[$project['project_id']] == 100000){
				...
				if (!empty($issuedToNameProjectId[$project['project_id']])){
					...
					$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM project_inspections as pi WHERE pi.project_id = '".$project['project_id']."' ".$locCondition.$roleProjectId[$project['project_id']]." AND pi.last_modified_date >= '".$lastModifiedDate."'");
					$inspId = array();		
					if(mysql_num_rows($rsIssuetoInspections) > 0){
						while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
							$inspId[] = $db->dataFilter($rowIssuetoInspections['inspection_id']);
				...
				if($statsRange[$project['project_id']] == "'ALL'"){
					...
					if($forStatus != ''){
						$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM project_inspections as pi, issued_to_for_inspections as isi WHERE pi.project_id = '".$project['project_id']."' ".$locCondition." AND DATE_SUB(CURDATE(), INTERVAL ".$dateRange[$project['project_id']]." DAY) <= pi.last_modified_date AND isi.inspection_status IN (".$forStatus.") AND pi.inspection_id = isi.inspection_id ".$roleProjectId[$project['project_id']] . $issuedToNameProjectId[$project['project_id']]." AND isi.last_modified_date >= '".$lastModifiedDate."'");
						if(mysql_num_rows($rsIssuetoInspections) > 0){
							while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
								$inspId2[] = $db->dataFilter($rowIssuetoInspections['inspection_id']);
							}
...
function subsequent_sync_refusedInpsectionIDS($projectId, $userId, $deviceType){
	...
	foreach($projectId as $project){
		$refuseIdsStr = '';
		$inspArray = $db->selQRYMultiple("inspection_id", "project_inspections", "last_modified_by = ".$userId." AND resource_type = '".$deviceType."' AND pi.last_modified_date BETWEEN '".$curTime['curTime']."' AND ('".$curTime['curTime']."' + INTERVAL 1 MINUTE) AND project_id = ".$project['project_id']);
...
function first_sync_metaData($table, $colString, $projectArray, $tableNameForIpad = '', $userId=''){
#echo $table.'============'.$colString.'============'.$projectArray;
	global $db;
	$queryArray = array();
	$selStr = $colString;
	$colArray = explode(',', $colString);
...
#	if($table == 'project_locations'){$colArray = array_merge(array('location_id'), $colArray);}
	if(count($queryArray) > 0){
		createJsonFile(EXPORTFILEPATH.'/download/', (!empty($tableNameForIpad)?$tableNameForIpad:$table), join(', ', $colArray), $queryArray);
	}
}
...
function first_sync_txtFile($table, $colString, $projectIDs){
	global $db;
	$queryArray = array();
	$selStr = $colString;
	$colArray = explode(',', $colString);
	$colArray = array_map('trim', $colArray);
	$loopCount = $recCount = sizeof($colArray);
	//$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." WHERE project_id IN (".$projectIDs.") AND is_deleted = 0");
	$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." AS up LEFT JOIN user AS u ON u.user_id = up.user_id WHERE project_id IN (".$projectIDs.") AND up.is_deleted = 0");
	$valString = '';
	if(mysql_num_rows($rs) > 0){
		while($row = mysql_fetch_assoc($rs)){			
...
function subsequent_sync_metaData_add($table, $colString, $projectArray, $new_project, $global_id, $tableNameForIpad = '', $userId=''){
	global $lastModifiedDate;
	global $db;
	$queryArray = array();
	$selStr = $colString;
	...
	if(count($queryArray) > 0){		
		$db->createFile((!empty($tableNameForIpad)?$tableNameForIpad:$table).'_add.txt', join('', $queryArray), EXPORTFILEPATH.'/download/');
		/*echo $table;
		echo '<pre>';print_r($queryArray);*/
	}
}
//Functin for Meta Data Update
function subsequent_sync_metaData_update($table, $colString, $projectArray, $tableNameForIpad = '', $userId=''){
	global $lastModifiedDate;
	global $db;
	global $deviceType;
	$queryArray = array();
	...
	if($table==='user_permission'){
		...
		if(mysql_num_rows($rs) > 0){
			while($row = mysql_fetch_assoc($rs)){
				...
				$valString .= "UPDATE ".(!empty($tableNameForIpad)?$tableNameForIpad:$table)." SET ".$valStr." WHERE global_id = ".$row[$updateID].";\r\n";
				$queryArray[] = $valString;
				$valString = '';
			}
		}
	...
	if(count($queryArray) > 0){
		if($deviceType == 'iPad' && $table == 'standard_defects'){
			$db->createFile('standard_deffects_update.txt', join('', $queryArray), EXPORTFILEPATH.'/download/');
		}else{
			$db->createFile((!empty($tableNameForIpad)?$tableNameForIpad:$table).'_update.txt', join('', $queryArray), EXPORTFILEPATH.'/download/');
		}
	}
}
...
function subsequent_sync_userProject_update($table, $colString, $userID){
	...
	if(mysql_num_rows($rs) > 0){
		while($row = mysql_fetch_assoc($rs)){
			...
			$valString .= "UPDATE ".$table." SET ".$valStr." WHERE global_id = ".$row[$updateID]." AND user_id = ".$userID.";\r\n";
			$queryArray[] = $valString;
			$valString = '';
		}
	}
...
//New Function for drawing_register Start Here
function first_sync_metaData_drawing_register($table, $colString, $projectArray, $global_id){
#echo $table.'++++'.$colString.'++++'.$projectArray.'++++'.$global_id;die;
	global $attributePemissionArr;
#echo $table.'============'.$colString.'============'.$projectArray;
	global $db;
	$queryArray = array();
	foreach($projectArray as $project){
		...
		$colArray = array_map('trim', $colArray);
		$loopCount = $recCount = sizeof($colArray);
#print_r($attributePemissionArr);
		$whereCon = "";
		$attr1Con = '';
/*		if($attributePemissionArr[$project['project_id']][0] != "")
			$attr1Con = 'attribute1 IN ('.$attributePemissionArr[$project['project_id']][0].') ';
		$attr2Con = '';
		if($attributePemissionArr[$project['project_id']][1] != "")
			$attr2Con = 'attribute1 IN ('.$attributePemissionArr[$project['project_id']][1].') ';
		...
		$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." WHERE project_id = '".$project['project_id']."' AND is_deleted = 0 ".$whereCon);
		$valString = '';
		if(mysql_num_rows($rs) > 0){
			while($row = mysql_fetch_assoc($rs)){
				$valStr = '';
...
function subsequent_sync_metaData_drawing_register_add($table, $colString, $projectArray, $new_project, $global_id){
	global $attributePemissionArr;
	global $lastModifiedDate;
	global $db;
	$queryArray = array();
...
/*		if($attributePemissionArr[$project['project_id']][0] != "")
		...
		$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." WHERE project_id = '".$project['project_id']."' AND last_modified_date >= '".$lastModifiedDate."' ".$whereCon);
		#}
		$valString = '';
		if(mysql_num_rows($rs) > 0){
			while($row = mysql_fetch_assoc($rs)){
...
function subsequent_sync_metaData_drawing_register_update($table, $colString, $projectArray){
	global $attributePemissionArr;
	global $lastModifiedDate;
	global $db;
	$queryArray = array();
...
/*		if($attributePemissionArr[$project['project_id']][0] != "")
		...
		*/
		...
		$rs = mysql_query("SELECT DISTINCT ".$selStr." FROM ".$table." WHERE last_modified_date >= '".$lastModifiedDate."' AND  project_id = '".$project['project_id']."' AND is_approved = 1 ".$whereCon);
...
//New Function for drawing_register End Here
