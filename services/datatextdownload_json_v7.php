...
$subSeqSync = $db->queryFilter(trim(strip_tags(addslashes($_REQUEST['subSeqSync']))));
//Upadted Dated : 04-09-2012
...
//Data for User Project Table
			$selUserProject = 'DISTINCT project_id, user_id, project_name, project_type, project_address_line1, project_address_line2, project_suburb, project_state, project_postcode, project_country, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
...
//Data for Checklist Name Table		
			$iPadQueryInspectionInspectedBy = array();
			foreach($projectId as $project){
				$selInspectionInspectedBy = 'check_list_items_id, project_id, check_list_items_name, check_list_items_tags, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
			...
			if(count($iPadQueryInspectionInspectedBy) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "check_list_items", 'global_id, project_id, check_list_items_name, check_list_items_tags, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted', $iPadQueryInspectionInspectedBy);
			}
...
//Data for Issued to for Inspections Table			
			foreach($projectId as $project){
				...
				if($statsRange[$project['project_id']] == "'ALL'"){
					$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM project_inspections as pi WHERE pi.project_id = '".$project['project_id']."' AND pi.is_deleted=0 AND DATE_SUB(CURDATE(), INTERVAL ".$dateRange[$project['project_id']]." DAY) <= pi.last_modified_date");
					$inspId = array();		
					if(mysql_num_rows($rsIssuetoInspections) > 0){
						while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
					...
					$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM project_inspections as pi, issued_to_for_inspections as isi WHERE pi.project_id = '".$project['project_id']."' AND pi.is_deleted=0 AND isi.inspection_status IN (".$forAll.") AND pi.inspection_id = isi.inspection_id AND isi.is_deleted = 0");				
					$inspId1 = array();		
					if(mysql_num_rows($rsIssuetoInspections) > 0){
						while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
...
//Data for add Project Inspections Table			
			foreach($projectId as $project){
				...
				$rsProjectInspections = mysql_query("SELECT ".$selProjectInspections." FROM project_inspections WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1)
				{
					$rsProjectInspections = mysql_query("SELECT ".$selProjectInspections." FROM project_inspections WHERE project_id = '".$project['project_id']."'and is_deleted=0");
				}
				if(mysql_num_rows($rsProjectInspections) > 0){
					$iPadQueryrsProjectInspections = array();
					while($rowProjectInspections = mysql_fetch_assoc($rsProjectInspections)){
...
//Data for Issued to for Inspections Table			
			foreach($projectId as $project){
				...
				$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM issued_to_for_inspections WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1)
				{
...
//Data for Inspection Graphics Table			
			foreach($projectId as $project){
				...
				$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_graphics WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1)
				{
...
//Data for Inspection Graphics Table			
			foreach($projectId as $project){
				...
				$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1)
				{
					$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."'and is_deleted=0");
				}
				if(mysql_num_rows($rsInspectionInspectedBy) > 0){
					$iPadQueryInspectionInspectedBy = array();
					while($rowInspectionInspectedBy = mysql_fetch_assoc($rsInspectionInspectedBy)){
