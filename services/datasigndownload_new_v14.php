...
//Code for Download the zip file
					...
					$zipFileName = $db->updateExportTable_updated(EXPORTFILEPATH.'/signoff/', $_REQUEST['userId'], $deviceType, 'signImages', $sync_url);
...
//new Project accepted condition GS Dated : 22-03-2013
			...
			if(!empty($noofFiles)){
				$imageCountQry = $db->selQRY('count(*) AS imageCount', 'project_inspections', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND inspection_id NOT IN ('.$inspDeSelect.') AND inspection_sign_image != "" AND is_deleted = 0');
				$imageCount = $imageCountQry['imageCount'];
				if($imageCount > 0){
					$output = array(
						'status' => true,
				...
				$imageName = $db->selQRYMultiple('inspection_sign_image, last_modified_date', 'project_inspections', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND inspection_id NOT IN ('.$inspDeSelect.') AND inspection_sign_image != "" AND is_deleted = 0 ORDER BY inspection_id ASC LIMIT '.$startIndex.', '.($imageLimit+1));
