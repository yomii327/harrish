...
#			$projectId = $db->getRecordByKey('user_projects', 'user_id', $userId, 'project_id');
			...
			if(!empty($noofFiles)){
				...
				$imageName = $db->selQRYMultiple('inspection_sign_image', 'project_inspections', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND inspection_sign_image != "" AND last_modified_date >= "'.$lastModifiedDate.'" AND is_deleted = 0 ORDER BY inspection_id ASC LIMIT '.$startIndex.', '.$imageLimit);
				$folder = opendir(SIGNSOURCEPATH);
				$pic_types = array("jpg", "jpeg", "gif", "png");
				$index = array();
				foreach($imageName as $imgName){
