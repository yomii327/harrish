...
//Data for Issued to for Inspections Table			
			...
			if($whereCon != ''){
				...
				$selIssuetoInspections = 'issued_to_inspections_id, project_id, inspection_id, issued_to_name, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, inspection_status, cost_attribute, inspection_fixed_by_date, closed_date, cost_impact_type';
			if($whereCon != ''){	
...
//Data for Issued to for Inspections Table			
			foreach($projectId as $project){
				$selIssuetoInspections = 'issued_to_inspections_id, project_id, inspection_id, issued_to_name, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, inspection_status, cost_attribute, inspection_fixed_by_date, cost_impact_type';
...
//Data for Update QA Task Locations
			foreach($projectId as $project){
				$selTaskLocation = 'DISTINCT location_id, project_id, location_parent_id, location_title, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
				$rsTaskLocation = mysql_query("SELECT ".$selTaskLocation." FROM draw_mgmt_images WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."'");
				if(mysql_num_rows($rsTaskLocation) > 0){
					$iPadQueryTaskLocation = array();
					while($rowTaskLocation = mysql_fetch_assoc($rsTaskLocation)){
						$rowTaskLocation['global_id'] = $rowTaskLocation['location_id'];
...
//Data for Update QA Task Monitoring
			foreach($projectId as $project){
				...
				if(mysql_num_rows($rsQATaskManatement) > 0){
					$iPadQueryQATaskManatement = array();
					while($rowQATaskManatement = mysql_fetch_assoc($rsQATaskManatement)){
						$rowQATaskManatement['global_id'] = $rowQATaskManatement['draw_mgmt_images_id'];
...
//Data for Update QA Inspections
		...
		$zipFileName = $db->updateExportTable_new(EXPORTFILEPATH . '/download/' . $zipFileName, $_REQUEST['userId'], $deviceType, 'tableData');
