...
$inspArray = array();
	foreach($projectId as $project){
		...
		if ($new_project[$project['project_id']] == 1){
			#echo "SELECT ".$selProjectInspections." FROM project_inspections  WHERE project_id = '".$project['project_id']."' ".$acceptConNewProj;die;
			...
			#echo "SELECT ".$selProjectInspections." FROM project_inspections  WHERE project_id = '".$project['project_id']."' ".$acceptCon." ".$refuseCon;die;
...
//inspection graphic table data here
		...
		if ($new_project[$project['project_id']] == 1){
			$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."' inspection_id IN (".$inspArray[$project['project_id']].")");
		}else{
			$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."' inspection_id IN (".$inspArray[$project['project_id']].")");
		}
...
//inspection_issue_to_add.txt Create Here
	subsequent_sync_metaData_add('inspection_issue_to', 'issue_to_name, last_modified_date, last_modified_by, created_date, created_by, resource_type, issue_to_id, project_id, is_deleted, tag, issue_to_email', $projectId, $new_project,"issue_to_id");
	
//inspection_issue_to_update.txt Create Here
	subsequent_sync_metaData_update('inspection_issue_to', 'issue_to_id, project_id, issue_to_name, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, tag, issue_to_email', $projectId);
