...
//Data for Progress Monitoring Table			
			...
			foreach($projectId as $project){
				$selProgressMonitoring = 'progress_id, project_id, location_id, sub_location_id, task, start_date, end_date, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, status, percentage, original_modified_date';
			...
			if(count($iPadQueryProgressMonitoring) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "progress_monitoring", 'global_id, project_id, location_id, sub_location_id, task, start_date, end_date, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, status, percentage, original_modified_date', $iPadQueryProgressMonitoring);
			}
...
//Data for Checklist Name Table		
			$iPadQueryInspectionInspectedBy = array();
			foreach($projectId as $project){
				$selInspectionInspectedBy = 'check_list_items_id, project_id, check_list_items_name, check_list_items_tags, check_list_items_option, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, issued_to, fix_by_days, checklist_type, holding_point';
			...
			if(count($iPadQueryInspectionInspectedBy) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "check_list_items", 'global_id, project_id, check_list_items_name, check_list_items_tags, check_list_items_option, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, issued_to, fix_by_days, checklist_type, holding_point', $iPadQueryInspectionInspectedBy);
			}
...
//Data for Issued to for Inspections Table			
			...
			foreach($projectId as $project){
				$selIssuetoInspections = 'location_check_list_id, project_id, location_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, original_modified_date';
			...
			if(count($locationChecklistData) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "location_check_list", 'global_id, project_id, location_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, original_modified_date', $locationChecklistData);
			}		
...
//Data for Insert Progres Monitoring Checklist Table			
			foreach($projectId as $project){
				$selIssuetoInspections = 'progress_monitoring_check_list_id, project_id, progress_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
...
//Data for Update Progres Monitoring Checklist Table				
			foreach($projectId as $project){
				if (!isset($new_project [$project['project_id']]))
				{
					$selIssuetoInspections = 'progress_monitoring_check_list_id, project_id, progress_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
...
//Data for location_check_list Table			
			foreach($projectId as $project){
				$selIssuetoInspections = 'location_check_list_id, project_id, location_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted, original_modified_date';
				$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM location_check_list WHERE project_id = '".$project['project_id']."' and last_modified_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1){
					$rsIssuetoInspections = mysql_query("SELECT ".$selIssuetoInspections." FROM location_check_list WHERE project_id = '".$project['project_id']."'and is_deleted=0");
				}
				if(mysql_num_rows($rsIssuetoInspections) > 0){
				...
				if (!isset($new_project [$project['project_id']])){
					...
					if(mysql_num_rows($rsIssuetoInspections) > 0){
						...
						while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
							...
							$iPadQueryInspectionInspectedBy[] = "update location_check_list set project_id=\"".$db->dataFilter($rowInspectionInspectedBy['project_id'])."\", location_id=\"".$db->dataFilter($rowInspectionInspectedBy['location_id'])."\", check_list_items_id=\"".$db->dataFilter($rowInspectionInspectedBy['check_list_items_id'])."\", check_list_items_status=\"".$db->dataFilter($rowInspectionInspectedBy['check_list_items_status'])."\", last_modified_date=\"".$db->dataFilter($rowInspectionInspectedBy['last_modified_date'])."\", last_modified_by=\"".$db->dataFilter($rowInspectionInspectedBy['last_modified_by'])."\", created_date=\"".$db->dataFilter($rowInspectionInspectedBy['created_date'])."\", created_by=\"".$db->dataFilter($rowInspectionInspectedBy['created_by'])."\", resource_type=\"".$db->dataFilter($rowInspectionInspectedBy['resource_type'])."\", is_deleted=\"".$db->dataFilter($rowInspectionInspectedBy['is_deleted'])."\", original_modified_date=\"".$db->dataFilter($rowInspectionInspectedBy['original_modified_date'])."\" where global_id=\"".$db->dataFilter($rowInspectionInspectedBy['global_id'])."\";\r\n";
