			...
			foreach($projectId as $project){
				...
				if($statsRange[$project['project_id']] == "'ALL'" && $dateRange[$project['project_id']] == 100000){
					...
					if($statsRange[$project['project_id']] == "'ALL'"){
						...
						$rsIssuetoInspections = mysql_query("SELECT inspection_id FROM inspection_graphics WHERE graphic_type = 'images' AND is_deleted=0 AND inspection_id IN (".join(",", $inspId).")");
						$inspId = array();		
						if(mysql_num_rows($rsIssuetoInspections) > 0){
							while($rowIssuetoInspections = mysql_fetch_assoc($rsIssuetoInspections)){
								$inspId[] = $db->dataFilter($rowIssuetoInspections['inspection_id']);
					...
					if($statsRange[$project['project_id']] == "'ALL'" && $dateRange[$project['project_id']] == 100000){
						...
						for($s=0;$s<$lupCount;$s++){
							if($insSelect == ''){
								...
								if (!empty ($inspectionIds[$project['project_id']][$s]))
									$insSelect .= ', '.$inspectionIds[$project['project_id']][$s];
							}
						}
...
#			$projectId = $db->getRecordByKey('user_projects', 'user_id', $userId, 'project_id');
			...
			if(!empty($noofFiles)){
				...
				$imageName = $db->selQRYMultiple('graphic_name, last_modified_date', 'inspection_graphics', 'project_id IN ('.$projId.')  AND inspection_id IN ('.$inspSelect.') AND graphic_type = "images"  AND last_modified_date >= "'.$lastModifiedDate.'" AND is_deleted = 0 ORDER BY inspection_id ASC LIMIT '.$startIndex.', '. ($imageLimit + 1));
				$folder = opendir(IMAGESOURCEPATH);
				$pic_types = array("jpg", "jpeg", "gif", "png");
				$index = array();
				$count = 0;
