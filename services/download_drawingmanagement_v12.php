...
//Add New Files
	...
	if(is_dir('../sync/Export/'.$globalId)){
		...
		if(empty($lastModifiedDate)){
			if(!empty($noofFiles)){
				...
				//for($i=0;$i<$loopCount;$i++){
					$imageName = $db->selQRYMultiple('project_id, draw_mgmt_images_id, draw_mgmt_images_name, draw_mgmt_images_thumbnail,last_modified_date', 'draw_mgmt_images', 'project_id IN ('.$projectIDs.') AND is_deleted = 0 ORDER BY draw_mgmt_images_id ASC LIMIT '.$startIndex.', '.$imageLimit);
			...
			if(!empty($noofFiles)){
				...
				//for($i=0;$i<$loopCount;$i++){
					$imageName = $db->selQRYMultiple('project_id, draw_mgmt_images_id, draw_mgmt_images_name, draw_mgmt_images_thumbnail, last_modified_date', 'draw_mgmt_images', 'project_id IN ('.$projectIDs.') AND last_modified_date >= "'.$lastModifiedDate.'" ORDER BY draw_mgmt_images_id ASC LIMIT '.$startIndex.', '.$imageLimit);
					$last_modified_graphic_date = "";
					if($isThumbnail == 'Y'){
						$folder = opendir($imgSource);
						$pic_types = array("jpg", "jpeg", "png", "JPG", "JPEG", "PNG");
		...
		if($db->emptyDirectory($zipSource)){
			...
			$zipFileName = $db->updateExportTable_new(EXPORTFILEPATH.'/'.$globalId.'/project_drawings/', $_REQUEST['userId'], $deviceType, 'DrawingMgmt');
