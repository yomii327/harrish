...
//Data for Inspection Checklist Data Table			
			...
			foreach($projectId as $project){
				...
				if(mysql_num_rows($rsInspectionInspectedBy) > 0){
					while($rowInspectionInspectedBy = mysql_fetch_assoc($rsInspectionInspectedBy)){
						...
						$iPadQueryInspectionInspectedBy[]  = " SELECT \"".$db->dataFilter($rowInspectionInspectedBy['insepection_check_list_id'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['project_id'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['inspection_id'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['check_list_items_id'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['check_list_items_status'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['last_modified_date'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['last_modified_by'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['created_date'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['created_by'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['resource_type'])."\", \"".$db->dataFilter($rowInspectionInspectedBy['is_deleted'])."\"";
			...
			if(count($iPadQueryInspectionInspectedBy) > 0){
				createJsonFile($db,EXPORTFILEPATH.'/download/', "inspection_check_list", 'global_id, project_id, inspection_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted', $iPadQueryInspectionInspectedBy);
			}
...
//Data for UpdateQuery User Project Table
			$selUserProject = 'DISTINCT project_id, user_id, project_name, project_type, project_address_line1, project_address_line2, project_suburb, project_state, project_postcode, project_country, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
			$rsUserProject = mysql_query("SELECT ".$selUserProject." FROM user_projects WHERE user_id = '".$_REQUEST['userId']."' and last_modified_date >= '".$lastModifiedDate."' and last_modified_date!=created_date");
			if(mysql_num_rows($rsUserProject) > 0){
				$iPadQueryUserProject = '';
				while($rowUserProject = mysql_fetch_assoc($rsUserProject)){
					#$rowUserProject['user_id'] = $userId;
...
//Data for Inspection Graphics Table			
			foreach($projectId as $project){
				...
				if(mysql_num_rows($rsInspectionInspectedBy) > 0){
					...
					$db->createFile('check_list_items_add.txt', $iPadQueryInspectionInspectedBy, EXPORTFILEPATH.'/download/');//Write File Here 
				}
			}
			...
			foreach($projectId as $project){
				if (!isset($new_project [$project['project_id']])){
					...
					if(mysql_num_rows($rsInspectionInspectedBy) > 0){
						...
						$db->createFile('check_list_items_update.txt', $iPadQueryInspectionInspectedBy, EXPORTFILEPATH.'/download/');//Write File Here 
					}
				}
			}
...
//Data for Inspection Graphics Table			
			foreach($projectId as $project){
				$selInspectionInspectedBy = 'insepection_check_list_id, project_id, inspection_id, check_list_items_id, check_list_items_status, last_modified_date, last_modified_by, created_date, created_by, resource_type, is_deleted';
				$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."' and created_date >= '".$lastModifiedDate."' and is_deleted=0");
				if ($new_project[$project['project_id']] == 1)
				{
					$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."'and is_deleted=0");
				}
				if(mysql_num_rows($rsInspectionInspectedBy) > 0){
					...
					$db->createFile('inspection_check_list_add.txt', $iPadQueryInspectionInspectedBy, EXPORTFILEPATH.'/download/');//Write File Here 
				}
			}
