...
if(isset($_REQUEST["antiqueID"])){
	if(isset($_POST['task'])){
		$select = "SELECT checklist_id, task_name FROM qa_project_checklist_task WHERE task_name = '".addslashes(trim($_POST['task']))."' AND project_checklist_id = '". $_POST['checklistId'] ."' AND is_deleted = 0";
		...
		// }else{
			...
			mysql_query("UPDATE qa_project_checklist_task SET ".$task_query." WHERE id=".$_POST['taskId']);
...
if(isset($_REQUEST["antiqueID"]) && isset($_REQUEST["deletetaskId"])){
	$update_task_query = "is_deleted = 1,
						...
						original_modified_date = NOW(),
						last_modified_by = ".$builder_id;
		//echo "UPDATE qa_project_checklist_task SET ".$update_task_query." WHERE id=".$_REQUEST["deletetaskId"]; die;
...
if(isset($_REQUEST["antiqueID"]) && isset($_REQUEST["taskId"])){
	...
	$taskData = $obj->selQRYMultiple('*', "qa_project_checklist_task", " is_deleted = '0' AND id = '".$_REQUEST["taskId"]."'");
	$taskData = $taskData[0];
		...
		<table width="550" border="0" align="left" cellpadding="0" cellspacing="15">
			...
			<tr style="color:#000000;">
				...
				<td align="left">
					...
					<input type="radio" name="comment_mandatory" value="No" <?php if($taskData['comment_mandatory'] == 'No'){echo 'checked="checked"'; }?> > No
				</td>
			</tr>

			<tr id="commentMandatory" style="color:#000000;">
			...
			<tr>
                ...
                <input type="button" name="button" class="green_small" id="button" style="float:left;" onclick="updateProjectTaskData(<?php echo $taskData['id']; ?>);" value="Submit"/>
                &nbsp;&nbsp;&nbsp; <a id="ancor" href="javascript:editProjectChecklistData(<?php echo $taskData['project_checklist_id']; ?>);" class="green_small" onclick="yes">Back</a>
				</td>
			</tr>
		</table>
		</form>
