...
if(isset($_POST['uniqueId'])){
	...
	$locData = $obj->selQRYMultiple('location_title', 'qa_task_locations', 'project_id = '.$_SESSION['idp'].' AND is_deleted = 0 AND location_parent_id = '.$locationId);
	$locCheckArr = array();
	if(!empty($locData)){
	...
	if(!in_array($location, $locCheckArr)){	
		$q="INSERT INTO qa_task_locations SET
		...
		$locNameTree = $obj->qa_sublocationParent($insertedId, ' > ');
		...
		$query = 'UPDATE qa_task_locations SET location_tree_name = "'.$locNameTree.'", last_modified_date = NOW(), last_modified_by = "'.$_SESSION['ww_builder_id'].'" WHERE location_id = '.$insertedId;
		mysql_query($query);
			...
			<table width="100%" border="0">
				...
				<tr>
					<td align="center" colspan="2" style="padding-top:20px;">
						<input type="button" name="submit" id="submit" value="Submit" onclick="addLocation()"  />
					</td>
				</tr>
			</table>
		</fieldset>
