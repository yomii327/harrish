...
//Get pdfData Here
	...
	if(empty($_FILES)){
		...
		$insertHistory = "INSERT INTO table_history_details SET
					primary_key = '".$_REQUEST['drawingRevID']."',
...
//Set Array for output data ids
	...
	//Title String Header
			...
			#if(!is_dir($d))
			#	mkdir($d);
			if(!is_dir('./report_pdf/'.$owner_id))	@mkdir('./report_pdf/'.$owner_id, 0777);
...
//Email and Message Board Entry Start Here
	...
	//Send Mail Start Here
				...
				//$mail->Send();
				if(!$mail->Send()) {
					$obj->save_email_status('drawing_register', $userEmailArrNew['user_email'],'', '', $msg, 0, $_SESSION['idp'], 'Document '.$drawingTitle.' updated in project '.$projectName, "attachment/".$name); 
				} else {
					$obj->save_email_status('drawing_register', $userEmailArrNew['user_email'],'', '', $msg, 1, $_SESSION['idp'], 'Document '.$drawingTitle.' updated in project '.$projectName, "attachment/".$name);
				}
				$mail->ClearAddresses();
	...
	<fieldset class="roundCorner">
		...
		<table width="550" border="0" align="left" cellpadding="0" cellspacing="15">
			...
			<tr>
				<td>&nbsp;</td>
				<td>
					<input type="submit" name="button" class="green_small" value="Save" id="button" style="float:left;" />
					&nbsp;&nbsp;&nbsp; <a id="ancor" href="javascript:void(0);" onclick="showRevisions(<?=$_REQUEST['regID']?>);" class="green_small">Back <!-- <img src="images/back_btn.png" style="border:none; width:111px;" /> --> </a>
				</td>
			</tr>
		</table>
		</form>
