...
if (isset($_SESSION["ww_is_builder"])){
	...
	if(!empty($_REQUEST['searchKeyward'])){
		...
		$_SESSION['reportData']['location_id_arr'] = $location_id_arr;
	...
	/*if($_REQUEST['issuedTo']!=""){
		...
		if(!empty($_REQUEST['searchKeyward'])){
			$where.=" and F.issued_to_name IN (".$mulIssueTo.")";
		}else{
...
if($report_type == "pdfSummayWithGPS"){$pageBreak = 5; $limit = ($pageBreak*10);}else{$pageBreak = 19; $limit = ($pageBreak*10);}	
	$qi="SELECT
...
if($noInspection > 0){
	...
	$toEmailIDS['reportType'] = 'Summary Report with GPS Details';
...
$html = '<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		if($offset == 0){
			$html .= '
				<tr>
					<td width="40%" style="font-size:14px;"><u><b>Summary Report with GPS Details</b></u></td>
					<td>&nbsp;</td>
				</tr>
				<tr>
			...
			if(!empty($_REQUEST['raisedBy']) && isset($_REQUEST['raisedBy'])){
				...
				<td width="200" style="font-size:11px;" valign="top">'.$_REQUEST['raisedBy'].'</td>';
				$jk++; if($jk%2 !=0){$html .= '';}if($jk%2 ==0){$html .= '</tr><tr>';}
			}
		...
		$html .= '</table><br />
				...
				<tr><td widht="3%" style="font-size:10px" align="center"><strong>ID</strong></td>
				<td widht="8%" style="font-size:10px" align="center"><strong>GPS Location</strong></td>';
	...
	$html .= '<td width="4%" style="font-size:10px;" align="center"><strong>Inspected By</strong></td>
			...
			<td width="4%" style="font-size:10px;" align="center"><strong>Close Date</strong></td>';
			if($report_type == "pdfSummayWithGPS"){
		...
		while($fi=mysql_fetch_assoc($ri)){$i++;
			...
			$issueToData = $obj->selQRYMultiple('issued_to_name, inspection_fixed_by_date, cost_attribute, inspection_status', 'issued_to_for_inspections', 'inspection_id = '.$fi['InspectionId'] . ' and is_deleted=0 ' . $where . $issToForInspWhere .' group by issued_to_name');
...
if($issueData['inspection_status'] == 'Closed'){ $isClosed = 'Closed'; }
	...
	$html .= '<tr><td widht="3%" style="font-size:10px" align="center">'.$fi["InspectionId"].'</td><td width="8%" style="font-size:10px" align="left">Lat : '.$fi["latitude"].'<br>Long : '.$fi["longitude"].'</td>';
	...
	$html .= '<td width="5%"><font style="font-size:10px;">'.stripslashes(wordwrap($fi["InspectedBy"], 15, "<br />", true)).'</font></td>
			...
			$closed_date = $fi["closedDate"] == "0000-00-00" ? "" : stripslashes(date("d/m/Y", strtotime($fi["closedDate"])));
			$html .= '</font></td>
			...
			<td width="4%" align="center"><font style="font-size:10px;">'.$closed_date.'</font></td>';
			if($report_type == "pdfSummayWithGPS"){
	...
	if ($i%$pageBreak == 0 && $i > ($pageBreak-1)){
		$html .='</tr></table><div class="footer" style="text-align:center;font-weight:bold;margin-top:5px;font-size:10px;" >DefectID \96 Copyright Wiseworking 2012 / 2013</div>
	...
	$html .= '<div class="footer" style="text-align:center;font-weight:bold;font-size:10px;" >DefectID \96 Copyright Wiseworking 2012 / 2013</div>';
