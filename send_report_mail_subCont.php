...
//Add BCC here
	...
	$issueArray = explode(',', $issueArr);
#3	print_r($attachmentArr);die;
//Add To here
	$toArr = explode(',', $to);
	for($i=0;$i<sizeof($toArr);$i++){
		$at = explode('/', $attachmentArr[$i]);
		$mail->AddAttachment('report_pdf/'.$_SESSION['ww_builder_id'].'/'.end($at));	
		$mail->AddAddress(trim($toArr[$i]), trim($issueArray[$i]));//send to mail heree
		$mail->Send();
