	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			if(imageName == 'images/checklist_btn2_red.png'){
				<?php for($g=1; $g <= $i; $g++){
					echo "document.getElementById('status_".$g."').innerHTML = '<option selected=\"selected\" value=\"Draft\">Draft</option>';";
				}?>
				checkListStatus = 1;
			}else
				if(imageName == 'images/checklist_btn2.png'){
					checkListStatus = 0;
					<?php for($g=1; $g <= $i; $g++){
						echo "document.getElementById('status_".$g."').innerHTML = '<option selected=\"selected\" value=\"Open\">Open</option><option value=\"Pending\">Pending</option><option value=\"Fixed\">Fixed</option><option value=\"Closed\">Closed</option>';";
					}?>
				}
		}
	}
...
function taggingIssueTo(locationName){
	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			if(xmlhttp.response != ''){
				var resString = xmlhttp.response;
...
<style>
label.spCheckbox { background: url(images/bullet.gif) no-repeat 0 0; float: left; font-family:Tahoma, Geneva, sans-serif; font-size:13px; text-align:left; padding-left: 25px; margin-right:5px; }
input[type=checkbox] { position: absolute; left: -999px; }
input[type=checkbox] + label.spCheckbox { display:block; width:70px; margin-top:10px; margin-left:8px; height: 18px; position:relative; }
input[type=checkbox]:checked + label.spCheckbox { background-image: url(images/bullet_checked.gif); }
</style>
<!-- Date Picker Starts -->
...
*/		$updateQryMul = "UPDATE issued_to_for_inspections SET
							...
							last_modified_by = '".$_SESSION['ww_builder']['user_id']."',
							inspection_status = '".$status."', last_modified_date = NOW()
						WHERE
							issued_to_inspections_id = '".$issue_to_id."'";
		mysql_query($updateQryMul);
	}?>
<!--<script language="javascript" type="text/javascript">window.location.href="?sect=show_defect_photo&id=<?=$_GET['did']?>&ms=<?=base64_encode('Updated')?>";</script>-->
...
while($rw = mysql_fetch_array($re)){	$val[] = $rw;	}?>
		...
		<table width="470" border="0" align="left" cellpadding="0" cellspacing="15" style="border: 1px solid;width: 670px;">
				<tr>
					...
					<input type="hidden" name="projectId" id="projectId" value="<?=$inspectionDetail['project_id']?>"  /></td>
				</tr>
				<tr>
					<td width="134" nowrap="nowrap" valign="top">Location</td>
					<td width="312" colspan="2"><?php 
								$locations_exists = $object->subLocations($inspectionDetail['location_id'], ' > ');?>
...
<?php $i=0; if(!empty($val)){foreach($val as $locations){$i++;?>
	<ul class="telefilms"><!-- Use 'cookie1' as unique key to save cookie only for this tree -->
		<li id="li_<?php echo $locations['location_id']?>"><span class="jtree-button demo1" id="<?php echo $locations['location_id']?>"><?php echo stripslashes($locations['location_title'])?></span>
