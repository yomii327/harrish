...
<style>
   ...
   #drawingDefault tbody { max-height: 500px !important; overflow:auto !important;}
	#middle
	{
		/*margin-top: 75px;*/
	}
	...
	tr.odd td:first-child, tr.even td:first-child, table#drawingDefault tr th:first-child
	{
		width:41px !important;
	}
	tr.odd td:nth-child(2n+0), tr.even td:nth-child(2n+0), table#drawingDefault tr th:nth-child(2n+0)
	{
		width:87px !important;
	}
	tr.odd td:nth-child(3n+0), tr.even td:nth-child(3n+0), table#drawingDefault tr th:nth-child(3n+0)
	{
		width:100px !important;
	}
	tr.odd td:nth-child(4n+0), tr.even td:nth-child(4n+0), table#drawingDefault tr th:nth-child(4n+0)
	{
		width:51px !important;
	}
	tr.odd td:nth-child(5n+0), tr.even td:nth-child(5n+0), table#drawingDefault tr th:nth-child(5n+0)
	{
		width:97px !important;
	}
	tr.odd td:nth-child(6n+0), tr.even td:nth-child(6n+0), table#drawingDefault tr th:nth-child(6n+0)
	{
		width:98px !important;
	}
	tr.odd td:nth-child(7n+0), tr.even td:nth-child(7n+0), table#drawingDefault tr th:nth-child(7n+0)
	{
		width:98px !important;
	}
	tr.odd td:nth-child(8n+0), tr.even td:nth-child(8n+0), table#drawingDefault tr th:nth-child(8n+0)
	{
		width:46px !important;
	}
	tr.odd td:nth-child(9n+0), tr.even td:nth-child(9n+0), table#drawingDefault tr th:nth-child(9n+0)
	{
		width:61px !important;
	}
	tr.odd td:nth-child(10n+0), tr.even td:nth-child(10n+0), table#drawingDefault tr th:nth-child(10n+0)
	{
		width:85px !important;
	}
	tr.odd td:nth-child(11n+0), tr.even td:nth-child(11n+0), table#drawingDefault tr th:nth-child(11n+0)
	{
		width:88px !important;
	}
	tr.odd td:nth-child(12n+0), tr.even td:nth-child(12n+0), table#drawingDefault tr th:nth-child(12n+0)
	{
		width:125px !important;
	}
	tr.odd td:last-child, tr.even td:last-child, table#drawingDefault tr th:last-child
	{
...
#editDrawingForm{
   ...
   table.display thead th div.DataTables_sort_wrapper {   /*padding-right: 16px;*/ padding-right: 10px;	}
   .big_container{
...
<div id="middle" <?php if($_GET['type'] == 'pmb'){?>class="gray_pmb" <? }?> style="padding-top:10px;margin-left: -100px;<?php echo $width;?>">
   ...
   <div id="rightCont"  style="<?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){ ?> width:700px;float:left;<? }else{?>width:980px;margin:0 auto;<?php }?> <?php if($_GET['view']=='workflow'){?>width:904px;<?php }?>">
      ...
      <div style="background:gray; ">
         <?php } ?>
		  <div id="errorHolder">
            <div class="success_r" id="outPutResult" style="height:35px;width:400px;display:none;">
               <p id="outPutResultPara"></p>
            </div>
         </div>	
         <ul class="headerHolder clearfix">
            ...
            <li style="text-align:right;padding-top:10px;">
               <form action="" name="projForm" id="projForm" method="post">
                  ...
                  <?php if(isset($_SESSION['ww_is_company'])){
                     echo $q = "SELECT project_id, project_name FROM user_projects WHERE is_deleted=0 and is_pdf = 1 GROUP BY project_name";
                     $res = mysql_query($q);
                     $prIDArr = array();
                     $outPutStr = "";
                     ...
                     $q = "SELECT project_id, project_name FROM user_projects WHERE user_id = '".$_SESSION['ww_builder_id']."' and is_deleted = 0 and is_pdf = 1 GROUP BY project_name";
					      //echo $q = "SELECT project_id, project_name FROM user_projects WHERE company_id IN(".$_SESSION['companyId'].") and is_deleted = 0 and is_pdf = 1 GROUP BY project_name";
                     $res = mysql_query($q);
                     $prIDArr = array();
                     $outPutStr = "";
                     while($q1 = mysql_fetch_array($res)){
         ...
         <div class="big_container" <?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){?>style="width:722px;"<? }?>>
            <div id="searchDraw" style=" display:block;  background:gray;<?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){?>width:690px;<? }?>border:1px solid <?php if($_GET['type'] == 'pmb'){echo '#818080';}?>;padding:5px;">
               <table width="100%" border="0">
                  <tr>
                     ...
                     <td style="padding-left:10px;<?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){echo '';}?>">Revision Number</td>
                     <td style="padding-left:10px;<?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){echo '';}?>">From Date</td>
                     <td style="padding-left:10px;<?php if(!isset($_GET['type']) && $_GET['type'] != 'pmb'){echo '';}?>">To Date</td>
                  ...
                  <tr>
                     ...
                     <td>
                      ...
                      <td>
                        <input type="text" name="searchRevision" id="searchRevision" class="input_small" style="width: 150px;
                           background-image: url(images/input_160.png);" />
                     </td>
                     <td>
                        <input type="text" name="fromDate" id="fromDate" class="input_small" style="width: 150px;
                           background-image: url(images/input_160.png);" />
                     </td>
                     <td>
                        <input type="text" name="toDate" id="toDate" class="input_small" style="width: 150px;
                           background-image: url(images/input_160.png);" />
                     </td>
                     ...
                     <?php }?>
                     <td>
                        <a class="green_small" href="javascript:void(0)" onclick="searchDrawPdf();" style="cursor:pointer; float:right;"  alt="search" />Search</a>
                     </td>
                  </tr>
                  <?php  if($_GET['view']=='workflow'){ ?>
            </div>
         ...
         <div id="toolbar">
            ...
            <a  onclick="downloadSelectedFiles(<?=$_SESSION['idp']?>);" class="actionButton green_small"  />Download Selected</a>   
            ...
            <a href="javascript:void(0)" onclick="addNewRegister();" class="actionButton green_small"  />Add New</a>
            <a href="javascript:void(0)"onclick="bulkUploadRegisters();" class="actionButton green_small"  />Bulk Upload</a>

            <a  onclick="downloadSelectedFiles(<?=$_SESSION['idp']?>);" class="actionButton green_small"  />Download Selected</a>   
            <?php if(isset($_GET['type']) && $_GET['type'] == 'pmb' && $_SESSION['userRole'] != 'General Consultant'){?>
            <a href="javascript:void(0)" onclick="completeAttachment(<?=$_SESSION['idp']?>);" class="actionButton green_small"  />Document Transmittal</a>
            <?php }else{ 
            ...
            <!-- <img src="images/document_register_btn.png" onclick="drawingRegisterReport(<?=$_SESSION['idp']?>);" class="actionButton"  /> -->
            <a href="javascript:void(0)" onclick="drawingRegisterReport(<?=$_SESSION['idp']?>);" class="actionButton"  />Document Register</a>
            <?php }else{?>
            <a onclick="downloadSelectedFiles(<?=$_SESSION['idp']?>);" class="actionButton green_small"  />Download Selected</a>	
            <?php }?>
            <?php if($_SESSION['ww_builder']['user_type'] == 'manager'){ ?>
            <!-- <a  href="javascript:void(0)" onclick="drawingRegisterReportManager(<?=$_SESSION['idp']?>);"  class="actionButton green_small" />Report</a> -->
            <?php }?>
            <!--<img src="images/approve.png" onclick="approvedSelected(<?=$_SESSION['idp']?>);" class="actionButton" />-->
            <a  href="javascript:void(0)" onclick="bulkApproval();" class="actionButton green_small" />Approve</a>

            <a  href="javascript:void(0)" class="actionButton green_small" onclick="mergeSelectedData()" <?php if(!isset($_SESSION['ww_is_builder']) || $_SESSION['ww_is_builder'] != 1){echo "style='display:none;'"; }?> >Merge</a>
            
           <a  href="pms.php?sect=o_and_m_workflow" class="actionButton green_small" <?php if(!isset($_SESSION['ww_is_builder']) || $_SESSION['ww_is_builder'] != 1){echo "style='display:none;'"; }?> >Workflow</a>
            <?php }?>
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
   	...
   	/*$('#drawingDefault').bind('mousewheel DOMMouseScroll', function (event) { 
   ...
   function searchDrawPdf(attrTab){
      ...
      var searchRevision = $('#searchRevision').val().trim();
      ...
      if(attrTab == 0 && drawingattribute1 == '' && drawingattribute2 == '' && searchKeyword == '' && pdfStatus == '' && onlyApproved == 0 && fromDate == ''){
      ...
      if(fromDate=='' && toDate!=''){
         ...
         ftd = fn_DateCompare(fromDate, toDate);
         if(ftd==true){
            jAlert('"To Date" cannot be less then "From Date".');
            return false;
         }
      }
   	...
   	<?php if(isset($_GET['type']) && $_GET['type'] == 'pmb'){?>
   		params = "req=pmb&attr1="+encodeURIComponent(drawingattribute1)+"&attr2="+encodeURIComponent(drawingattribute2)+"&searchKey="+encodeURIComponent(searchKeyword)+"&searchRevision="+encodeURIComponent(searchRevision)+"&fromDate="+encodeURIComponent(fromDate)+"&toDate="+encodeURIComponent(toDate)+"&pdfStatus="+encodeURIComponent(pdfStatus)+"&name="+Math.random();
   	<?php }else{?>
   		params = "attr1="+encodeURIComponent(drawingattribute1)+"&attr2="+encodeURIComponent(drawingattribute2)+"&searchKey="+encodeURIComponent(searchKeyword)+"&searchRevision="+encodeURIComponent(searchRevision)+"&fromDate="+encodeURIComponent(fromDate)+"&toDate="+encodeURIComponent(toDate)+"&pdfStatus="+encodeURIComponent(pdfStatus)+"&name="+Math.random();
   	<?php }?>
   	if(attrTab != 0){
   		<?php if(isset($_GET['type']) && $_GET['type'] == 'pmb'){?>
   			params = "req=pmb&attrTab="+encodeURIComponent(attrTab)+"&attr1="+encodeURIComponent(drawingattribute1)+"&attr2="+encodeURIComponent(drawingattribute2)+"&searchKey="+encodeURIComponent(searchKeyword)+"&searchRevision="+encodeURIComponent(searchRevision)+"&fromDate="+encodeURIComponent(fromDate)+"&toDate="+encodeURIComponent(toDate)+"&pdfStatus="+encodeURIComponent(pdfStatus)+"&name="+Math.random();
   		<?php }else{?>
   			params = "attrTab="+encodeURIComponent(attrTab)+"&attr1="+encodeURIComponent(drawingattribute1)+"&attr2="+encodeURIComponent(drawingattribute2)+"&searchKey="+encodeURIComponent(searchKeyword)+"&searchRevision="+encodeURIComponent(searchRevision)+"&fromDate="+encodeURIComponent(fromDate)+"&toDate="+encodeURIComponent(toDate)+"&pdfStatus="+encodeURIComponent(pdfStatus)+"&name="+Math.random();
   		<?php }?>
   	}
      console.log(params);
   	$('#SearchTabsHide').html(attrTabs);
      ...
      var idCreate = attrTab.replace(/ /g,'');
      ...
      $(".allElements").removeClass("attribute_active");
      $("#"+idCreate+"_id").addClass("attribute_active");
   }
...
<script type="text/javascript" src="js/multiupload_v1.js?version=<?=$js_version?>"></script>
<script type="text/javascript" src="js/upload_v1.js?version=<?=$js_version?>"></script>
<script type="text/javascript" src="js/upload_DT.js"></script>
<script type="text/javascript" src="js/upload_edit_reivison_v1.js?version=<?=$js_version?>"></script>
...
<script>
   ...
   function fn_DateCompare(DateA, DateB) {     // this function is good for dates > 01/01/1970
      ...
      //var foo = "09/22/2011";
      var fooA = DateA; //"09/22/2011";
      var arrA = fooA.split("-");
   
      fromD = arrA[2]+"/"+arrA[1]+"/"+arrA[0];

      var fooA = DateB; 
      var arrB = fooA.split("-");

      toD = arrB[2]+"/"+arrB[1]+"/"+arrB[0];
      
      return new Date(toD).valueOf() <= new Date(fromD).valueOf();
      //return new Date(fromD).valueOf() < new Date(toD).valueOf();
   }
