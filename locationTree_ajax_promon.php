...
$location_query = "SELECT location_id, location_title FROM project_monitoring_locations WHERE project_id = '".$_SESSION['idp']."' AND location_parent_id = '".$_POST['cetid']."' AND is_deleted = '0' ORDER BY location_title";
if($_POST['cetid'] == 0){
	$class = 'telefilms';
}else{
	$class = '';
...
if($isLocation > 0){
	...
	<ul  class="<?=$class?>" style="display:none" >
		<li id="li_<?=$locations['location_id']?>">
			<?php $data = $obj->recurtionProMon($locations['location_id'], $_SESSION['idp']);
			if($data != ''){ ?>
