...
if(isset($_REQUEST['confirm'])){
	if(!empty($all_categories)){
		...
		foreach($all_categories as $deleteId){
			$query = "UPDATE project_locations SET is_deleted = '1', last_modified_by = '".$_SESSION['ww_builder_id']."', last_modified_date = NOW() WHERE project_id = '".$_SESSION['idp']."' and location_id = '".$deleteId."'";
		...
		if($deleteCount > 0){
			if(!empty($all_categories)){
				...
				$deleteQry = "UPDATE project_inspections SET is_deleted = '1', last_modified_by = '".$_SESSION['ww_builder_id']."', last_modified_date = NOW(), original_modified_date = NOW() WHERE project_id = '".$_SESSION['idp']."' AND inspection_id IN (".$locIDs[0]['inspectionids'].")";
				mysql_query($deleteQry);
			
				$deleteIssueQry = "UPDATE issued_to_for_inspections SET is_deleted = '1', last_modified_by = '".$_SESSION['ww_builder_id']."', last_modified_date = NOW(), original_modified_date = NOW() WHERE project_id = '".$_SESSION['idp']."' AND inspection_id IN (".$locIDs[0]['inspectionids'].")";
				mysql_query($deleteIssueQry);
			
				$deleteGraphicQry = "UPDATE inspection_graphics SET is_deleted = '1', last_modified_by = '".$_SESSION['ww_builder_id']."', last_modified_date = NOW(), original_modified_date = NOW() WHERE project_id = '".$_SESSION['idp']."' AND inspection_id IN (".$locIDs[0]['inspectionids'].")";
