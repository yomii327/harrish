...
if(isset($_REQUEST["antiqueID"])){
  if(isset($_POST['allTask']) && !empty($_POST['allTask'])){
    ...
    foreach ($allTask as $value) {
      ...
      //echo "UPDATE qa_checklist_task SET ".$task_query." WHERE id=".$value;
      mysql_query("UPDATE qa_checklist_task SET ".$task_query." WHERE id=".$value);
      $order_id++;
    }
    //die();
...
if(isset($_REQUEST["antiqueID"]) && isset($_REQUEST["checklistId"])){
  ...
  $taskData = $obj->selQRYMultiple('*', "qa_checklist_task", " is_deleted = '0' AND checklist_id = '".$_REQUEST["checklistId"]."' ORDER BY order_id asc");
  $allTask = $taskData;
  $taskData = $taskData[0];
    ...
    <table width="550" border="0" align="left" cellpadding="0" cellspacing="15">
      ...
      <tr>
        ...
        <td>
          ...
          <input type="button" name="button" class="submit_btn" id="button" style="background-image:url(images/submit.png);font-size:0px; border:none; width:111px;float:left;" onclick="saveOrderFunc(<?php echo $taskData['checklist_id']; ?>);" />
                &nbsp;&nbsp;&nbsp; 
        </td>
      </tr>
    </table>
